...
import com.nuaa.ec.scienresearch.exportdata.ExportScienReschSummaryDataExcel;
...
public class ScientificResearchSummaryDataDao {
	 ...
	 * 科研模块汇总数据的导出
	 */
	public static ByteArrayOutputStream getExcelOutputStream(String researchLabId,
			String foredate, String afterdate) throws IOException{
		@SuppressWarnings("unchecked")
		List<ScientificResearchModuleData> scienReschModuleDataList = (List<ScientificResearchModuleData>) ActionContext
				.getContext().getSession().get("scienReschModlSumryData");
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		ExportScienReschSummaryDataExcel.generateSummaryDataExcel(scienReschModuleDataList, researchLabId, foredate, afterdate).write(baos);
		return baos;
	}
	...
	public static List<ScientificResearchModuleData> summaryScienReschModuleDataByPerson(
			String queryTeacher, String foredate, String afterdate)
			throws Exception {
		...
		List<Teacher> teachers = new TeacherDAO()
				.findTeacherByFuzzyQuery(queryTeacher);
		for (Teacher teacher : teachers) {
			scienReschModuleData = new ScientificResearchModuleData();
			scienReschModuleData.setTeacherId(teacher.getTeacherId());
			scienReschModuleData.setTeacherName(teacher.getTeacherName());
	...
	public static void getEachModuleAllResearchLab(
		...
		float scienReschProColumn = 0, scienReschRewardColumn = 0, academicWorkColumn = 0, joinAcademicMeetingColumn = 0, selectTalentProColumn = 0, inviteExpertSpeechColumn = 0, periodicalColumn = 0, mainUndertakeAcademicMeetingColumn = 0;
		for (ScientificResearchModuleData srmd : scienResearchModlDataList) {
			...
			joinAcademicMeetingColumn += srmd.getJoinAcademicMeetingData()
					.getSum();
			selectTalentProColumn += srmd.getTalentProData().getSum();
			inviteExpertSpeechColumn += srmd.getInviteExpertSpeechData()
					.getSum();
			periodicalColumn += srmd.getPeriodicalData().getSum();
			mainUndertakeAcademicMeetingColumn += srmd
					.getUndertakeAcademicMeetingData().getSum();
		}
		...
		scienReschProData.setAvg(NumberFormatUtil
				.getNumberAfterTransferPrecision(scienReschProColumn / size));
		// 科研奖励
		scienReschRewardData.setSum(scienReschRewardColumn);
		scienReschRewardData
				.setAvg(NumberFormatUtil
						.getNumberAfterTransferPrecision(scienReschRewardColumn
								/ size));
		// 学术著作
		academicWorkData.setSum(academicWorkColumn);
		academicWorkData.setAvg(NumberFormatUtil
				.getNumberAfterTransferPrecision(academicWorkColumn / size));
		// 参加学术会议
		joinAcademicMeetingData.setSum(joinAcademicMeetingColumn);
		joinAcademicMeetingData.setAvg(NumberFormatUtil
				.getNumberAfterTransferPrecision(joinAcademicMeetingColumn
						/ size));
		// 入选人才工程
		selectTalentProData.setSum(selectTalentProColumn);
		selectTalentProData.setAvg(NumberFormatUtil
				.getNumberAfterTransferPrecision(selectTalentProColumn / size));
		// 邀请专家讲学
		inviteExpertSpeechData.setSum(inviteExpertSpeechColumn);
		inviteExpertSpeechData.setAvg(NumberFormatUtil
				.getNumberAfterTransferPrecision(inviteExpertSpeechColumn
						/ size));
		// 期刊论文
		periodicalData.setSum(periodicalColumn);
		periodicalData.setAvg(NumberFormatUtil
				.getNumberAfterTransferPrecision(periodicalColumn / size));
		// 主承担学术会议
		undertakeAcademicMeetingData.setSum(mainUndertakeAcademicMeetingColumn);
		undertakeAcademicMeetingData
				.setAvg(NumberFormatUtil
						.getNumberAfterTransferPrecision(mainUndertakeAcademicMeetingColumn
								/ size));
		...
		scienReschModuleData
				.setUndertakeAcademicMeetingData(undertakeAcademicMeetingData);
		scienReschModuleData.setSum(academicWorkData.getSum()
				+ inviteExpertSpeechData.getSum()
		...
		scienReschModuleData
				.setAvg(NumberFormatUtil
						.getNumberAfterTransferPrecision(scienReschModuleData
								.getSum() / 8));
