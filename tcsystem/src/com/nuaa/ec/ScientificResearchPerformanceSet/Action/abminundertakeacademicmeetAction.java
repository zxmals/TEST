...
//	DAO-attr
	...
	public void updateUnderacademic()throws Exception{
		...
		try {
			...
			if(undertakemeetscore!=null){
				...
				ServletActionContext.getResponse().getWriter().write("会议地点与回忆类型无对应评分项");
				return ;
			}
	...
	public void deleteMember()throws Exception{
		...
		try {
			...
			if(!undertakemeet.getChargePersonId().trim().equals(teacherandudtmeet.getTeacher().getTeacherId())){
				teacherandudtmeetdao.deleteBylogic(teacherandudtmeet.getTeacher(), undertakemeet);
			}else{
				ServletActionContext.getResponse().setCharacterEncoding("utf-8");
