...
public class FineCourseConstructionExcel {
	/**
	 * function：导出精品课程数据
	 * @param ths 导出数据的字段
	 * @param foredate 下限日期
	...
	public static HSSFWorkbook  generateExcel(String[] ths,List<TffineCourseConstructionPerformance> tfFineCorsConsPerfList,String researchLabName,String foredate,String afterdate){
...
//  合并单元格  org.apache.poi.hssf.util.Region(short rowFrom,short colFrom,short rowTo,short colTo)
		...
		cells[0].setCellValue("精品课程建设["+researchLabName+"]");
		//应用格式
...
/**
 * 导出字段：
 * {"课程编号","课程名称","课程等级","是否合作","项目总分","负责人工号","负责人姓名","当前教师工号","当前教师姓名","本人承担任务","教师绩效","学期"};
 */
				cell[0].setCellValue(tfFineCorsConsPerfList.get(i).getTffineCourseConstructionProject().getCourseId());
				cell[0].setCellStyle(cellStyle);
				cell[1].setCellValue(tfFineCorsConsPerfList.get(i).getTffineCourseConstructionProject().getCourseName());
				...
				cell[5].setCellValue(tfFineCorsConsPerfList.get(i).getTffineCourseConstructionProject().getChargePersonId());
				cell[5].setCellStyle(cellStyle);
				cell[6].setCellValue((String)teachers.get(tfFineCorsConsPerfList.get(i).getTffineCourseConstructionProject().getChargePersonId()));
				cell[6].setCellStyle(cellStyle);
				cell[7].setCellValue(tfFineCorsConsPerfList.get(i).getTeacher().getTeacherId());
				cell[7].setCellStyle(cellStyle);
				cell[8].setCellValue(tfFineCorsConsPerfList.get(i).getTeacher().getTeacherName());
				cell[8].setCellStyle(cellStyle);
				cell[9].setCellValue(tfFineCorsConsPerfList.get(i).getSelfUndertakeTask().getUndertakeTaskName());
				cell[9].setCellStyle(cellStyle);
				cell[10].setCellValue(tfFineCorsConsPerfList.get(i).getSingelScore());
				cell[10].setCellStyle(cellStyle);
				cell[11].setCellValue(tfFineCorsConsPerfList.get(i).getTffineCourseConstructionProject().getTfterm().getTerm());
