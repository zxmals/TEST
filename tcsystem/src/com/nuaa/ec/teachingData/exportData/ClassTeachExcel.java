...
public class ClassTeachExcel {
	/**
	 * function：导出课堂教学数据
	 * @param ths 导出数据的字段
	 * @param tfClassTeachPfomsList Excel表对应的数据实体类
	...
	public static HSSFWorkbook  generateExcel(String[] ths,List<TfclassTeachPefromanceUnionTfterm> tfClassTeachPfomsList,String researchLabName,String foredate,String afterdate){
...
//  合并单元格  org.apache.poi.hssf.util.Region(short rowFrom,short colFrom,short rowTo,short colTo)
		...
		cells[0].setCellValue("课堂教学["+researchLabName+"]");
		//应用格式
		...
		if(tfClassTeachPfomsList!=null){
			for(int i=0;i<tfClassTeachPfomsList.size();i++){
 ...
 */
				...
				cell[1].setCellValue(tfClassTeachPfomsList.get(i).getClassTeachPerformance().getSumtime());
				cell[1].setCellStyle(cellStyle);
				cell[2].setCellValue(tfClassTeachPfomsList.get(i).getClassTeachPerformance().getTfclassTeachEvaluation().getEvaluResult());
				cell[2].setCellStyle(cellStyle);
				cell[3].setCellValue(tfClassTeachPfomsList.get(i).getClassTeachPerformance().getTeacher().getTeacherId());
				cell[3].setCellStyle(cellStyle);
				cell[4].setCellValue(tfClassTeachPfomsList.get(i).getClassTeachPerformance().getTeacher().getTeacherName());
