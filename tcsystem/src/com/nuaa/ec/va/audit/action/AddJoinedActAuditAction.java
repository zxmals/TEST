...
public class AddJoinedActAuditAction implements RequestAware, SessionAware {
	public void doCheckOutTask() {
		...
		for (int i = 0; i < ids.length; i++) {
			if (ids[i] != null && ids[i].length() != 0) {
				// VateacherAndCollectiveActId vateacherAndCollectiveActId = new
				// VateacherAndCollectiveActId(
				// new VacollectiveActivitiesPublishDAO().findById(ids[i]),
				// new TeacherDAO().findById(new
				// VacollectiveActivitiesPublishDAO().findById(ids[i]).getTeacherId())
				// );
		...
		for (int i = 0; i < idsNot.length; i++) {
			if (idsNot[i] != null && idsNot[i].length() != 0) {
				...
				// new TeacherDAO().findById(new
				// VacollectiveActivitiesPublishDAO().findById(idsNot[i]).getTeacherId())
				// );
				VateacherAndCollectiveActId vateacherAndCollectiveActId = new VateacherAndCollectiveActId(
	...
	public String getAddJoinedActListAfterDivide() {
		...
		try {
			this.request.put("VaAddJoinedActList", this.vateacherAndCollectiveActDAO.getVaAddJoinedActAfterDivide(
					pageIndex, (Integer) session.get("pageSize_CT"), (String) session.get("foredate_CT"),
					(String) session.get("afterdate_CT"), (Department) session.get("department_CT"),
	...
	public String getAddJoinedActList() {
		...
		try {
			this.request.put("VaAddJoinedActList", this.vateacherAndCollectiveActDAO.getVaAddJoinedAct(pageIndex,
					(Integer) session.get("pageSize_CT"), (String) session.get("foredate_CT"),
					(String) session.get("afterdate_CT"), (Department) session.get("department_CT"),
					(String) session.get("checkOutStatus_CT")));
			this.setOperstatus(1);
		} catch (Exception e) {
