...
public class NewActApplyAudit implements RequestAware, SessionAware {
	...
	public String getNewActApplyList() {
		...
		try {
			this.request.put("newActApplyList", this.vacollectiveActDAO.getNewActApplyList(pageIndex,
					(Integer) session.get("pageSize_CT"), (Department) session.get("department_CT"),
					(String) session.get("checkOutStatus_CT"), false));
			tx.commit();
	...
	public String getNewActApplyListAfterDivided() {
		...
		try {
			this.request.put("newActApplyList", this.vacollectiveActDAO.getNewActApplyList(pageIndex,
					(Integer) session.get("pageSize_CT"), (Department) session.get("department_CT"),
					(String) session.get("checkOutStatus_CT"), true));
			tx.commit();
			this.setOperstatus(1);
		} catch (Exception e) {
			// TODO: handle exception
