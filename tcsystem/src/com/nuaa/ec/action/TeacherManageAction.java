...
//		Teacher teacher = null;
		 ...
		 * check user be or not fisrt log in
		 * check user be or not admin 
		 */
		...
		if(teacherlg!=null){
			...
			if(teacher!=null){
				...
				if(password.equals(teacher.getTeacherId())){
					...
					/*第一次登录 初始默认信息: level:1(普通教师) logintime: 1(登录次数)/ first login default-info:level:1(general-teacher),logintime:1*/
					teacherlg = new TeacherLoginInfo(teacherid, teacher, password, "1", "1", 1, new java.sql.Timestamp(new java.util.Date().getTime()));
					teacherlgdao.save(teacherlg);
					session.put("teacher", teacher);
					loginresult = "general-t";
				}
				else{
					System.out.println("error password.first login password==ID");
		...
		new BaseHibernateDAO().closeSession(new BaseHibernateDAO().getSession());
		return loginresult;
	}
