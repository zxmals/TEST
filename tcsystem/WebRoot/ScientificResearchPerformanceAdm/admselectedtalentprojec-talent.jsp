...
<body class="gray-bg"  onload="DoCheck()">
	<div class="datepick">
		<span>选择日期范围</span>
		<div>
			<form action="ATselectedtalentprojec-talentset!gainAlltalentprojec?pagenum=1" method="post" name="pickdate" id="pickdates">
	                    ...
	                    <br>
	                       ...
	                       <table  id="tb" class="table table-striped table-bordered table-hover dataTables-example">
	                        ...
	                        <div style="text-align: center;">
	                        	(共查询到${sumrow }条记录)&nbsp;&nbsp;&nbsp;&nbsp;
	                        	第${pagenum }/${sumpage }页&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="ATselectedtalentprojec-talentset!gainAlltalentprojec?pagenum=1">首页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="ATselectedtalentprojec-talentset!gainAlltalentprojec?pagenum=${prepage }">上一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="ATselectedtalentprojec-talentset!gainAlltalentprojec?pagenum=${nextpage }">下一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="ATselectedtalentprojec-talentset!gainAlltalentprojec?pagenum=${sumpage }">尾页</a>
	                        </div>
	                   </div>
	                </div>
	            </div>
    ...
    $('#changelength').change(function() {
		...
		window.location.replace("ATselectedtalentprojec-talentset!gainAlltalentprojec?pagenum=1&limit="+$(this).val().trim()+"&foredate="+$('#date1').val().trim()+"&afterdate="+$('#date2').val().trim());
	});
    </script>
    <!-- update -->
    <script type="text/javascript">
    ...
    $('.carrydata').click(function() {
		...
		$('input[type="radio"][name="proJpeople"][value="'+(row[0].cells[5].title.trim()=="0"?"0":"1")+'"]').prop("checked",true);
		$('input[type="radio"][name="proJpeople"]:checked').prop("value",row[0].cells[5].title.trim());
		chargePersonIds = row[0].cells[3].innerHTML;
	});
    	...
    	if(checkadds()&$('#talentProjectId').val().trim()!=""){
    			swal({   
    	    		...
    	    		closeOnCancel: true }, 
    	    			function(isConfirm){   
    	    				if (isConfirm) {
    	    					$.post("ATselectedtalentprojec-talentset!updateTalentPeoject",
    	    						 	{"talentp.talentProjectId":$('#talentProjectId').val().trim(),
    	    	    					 "talentp.talentProjectName":$('#projectname').val().trim(),
    	    	    					 "talentp.chargePersonId":chargePersonIds,
    	    	    					...
    	    	    					function(data,status){
    	    	    						if(status=="success"){
    	    	    							 if(data=="succ"){
    	    	    								 swal("更新成功","","success");
    	    	    								 setTimeout(function() {
    	    	    									 window.location.replace("ATselectedtalentprojec-talentset!gainAlltalentprojec?pagenum=1");
    	    										}, 2000);
    	    	    							 }else{
    ...
    $('#subdel').click(function() {
    	swal({   
    		...
    		closeOnCancel: true }, 
    			function(isConfirm){   
    				if (isConfirm) {
    					$.post("ATselectedtalentprojec-talentset!deleteTalentProjec",
    							{"talentp.talentProjectId":$('#talentProjectId').val().trim()},
    							function(data,status){
    								if(status=="success"){
    									if(data=="succ"){
    ...
    $('.getMember').click(function() {
		var row = $(this).parent().parent();
		$.post("ATselectedtalentprojec-talentset!getMember",
        		...
        		closeOnCancel: true }, 
        			function(isConfirm){   
        				if (isConfirm&pubsprojectId!="") {
        					$.post("ATselectedtalentprojec-talentset!deleteMember",
        							{"talentp.talentProjectId":pubsprojectId,
        						 	 "teacherandtalentp.teacher.teacherId":row[0].cells[0].innerHTML.trim()},
        							function(data,status){
        								if(status=="success"){
        									if(data=="succ"){
        										swal("已移除","","success");
