...
<body class="gray-bg" onload="DoCheck()">
								...
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${entityList }">
											<tr>
												<td>${ebj.thesisRetrivalId }</td>
												<td>${ebj.author}</td>
												<td>${ebj.thesisRetrieval}</td>
												<td>${ebj.score }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>作者:</label> <input id="upinfscope" type="text"
									class="form-control nullcheck" name="" value="">
							</div>
							<div class="form-group">
								<label>论文等级:</label> <input id="upinfthesisRetrieval" type="text"
									class="form-control nullcheck" name="" value="">
							</div>
							<div class="form-group">
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATteachPaperRetrievalConditionset!addEntity" method="post">
								<div class="form-group">
									<label>作者:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="entity.author"
										value="">
								</div>
								<div class="form-group">
									<label>论文等级:</label> <input id="addinf2" type="text"
										class="form-control nullcheck"
										name="entity.thesisRetrieval" value="">
								</div>
								<div class="form-group">
									<label>系数:</label> <input id="addinf3" type="text"
										class="form-control nullcheck" name="entity.score"
										value="">
								</div>
							</form>
							<div>
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATteachPaperRetrievalConditionset!updateEntity",
					{"entity.thesisRetrivalId":$('#upinfID').val().trim(),
					 "entity.author":$('#upinfscope').val().trim(),
					 "entity.thesisRetrieval":$('#upinfthesisRetrieval').val().trim(),
					 "entity.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATteachPaperRetrievalConditionset!entityList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATteachPaperRetrievalConditionset!deleteEntity",
							{"entity.thesisRetrivalId":row[0].cells[0].innerHTML,
							 "entity.author":row[0].cells[1].innerHTML,
							 "entity.thesisRetrieval":row[0].cells[2].innerHTML,
							 "entity.sclore":row[0].cells[3].innerHTML},
							function(data,status){
								if(status=="success"){
									if(data.trim()=="succ"){
    ...
    $('.openupdatem').click(function() {
		...
		$('#upinfthesisRetrieval')[0].value=$(this).parent().parent()[0].cells[2].innerHTML;
		$('#upinfratio')[0].value=$(this).parent().parent()[0].cells[3].innerHTML;
