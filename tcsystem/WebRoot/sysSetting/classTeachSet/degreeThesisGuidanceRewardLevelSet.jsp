...
<body class="gray-bg" onload="DoCheck()">
						...
						<div class="example">
							<form method="post" name="f">
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${degreeThesisGuidanceRewardLevelList}">
											<tr>
												<td>${ebj.rewardLevelId}</td>
												<td>${ebj.rewardLevel}</td>
												<td>${ebj.score }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>论文等级:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATdegreeThesisGuidanceRewardLevelset!adddegreeThesisGuidanceRewardLevel" method="post">
								<div class="form-group">
									<label>论文等级:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="degreeThesisGuidanceRewardLevel.rewardLevel"
										value="">
								</div>
								<div class="form-group">
									<label>系数:</label> <input id="addinf2" type="text"
										class="form-control nullcheck"
										name="degreeThesisGuidanceRewardLevel.score" value="">
								</div>
							</form>
							<div>
								<button type="button"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATdegreeThesisGuidanceRewardLevelset!updatedegreeThesisGuidanceRewardLevel",
					{"degreeThesisGuidanceRewardLevel.rewardLevelId":$('#upinfID').val().trim(),
					 "degreeThesisGuidanceRewardLevel.rewardLevel":$('#upinfscope').val().trim(),
					 "degreeThesisGuidanceRewardLevel.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATdegreeThesisGuidanceRewardLevelset!degreeThesisGuidanceRewardLevelList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATdegreeThesisGuidanceRewardLevelset!deletedegreeThesisGuidanceRewardLevel",
							{"degreeThesisGuidanceRewardLevel.rewardLevelId":row[0].cells[0].innerHTML,
							 "degreeThesisGuidanceRewardLevel.rewardLevel":row[0].cells[1].innerHTML,
							 "degreeThesisGuidanceRewardLevel.score":row[0].cells[2].innerHTML},
							function(data,status){
								if(status=="success"){
									if(data.trim()=="succ"){
