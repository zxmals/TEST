...
<body class="gray-bg" onload="DoCheck()">
								...
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${entityList }">
											<tr>
												<td>${ebj.courseLevelId }</td>
												<td>${ebj.courseLevel}</td>
												<td>${ebj.score }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATfineCourseConstructionLevelset!addEntity"
								method="post">
								<div class="form-group">
									<label>课程等级:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="entity.courseLevel"
										value="">
								</div>
								<div class="form-group">
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATfineCourseConstructionLevelset!updateEntity",
					{"entity.courseLevelId":$('#upinfID').val().trim(),
					 "entity.courseLevel":$('#upinfscope').val().trim(),
					 "entity.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATfineCourseConstructionLevelset!entityList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATfineCourseConstructionLevelset!deleteEntity",
							{"entity.courseLevelId":row[0].cells[0].innerHTML,
							 "entity.courseLevel":row[0].cells[1].innerHTML,
