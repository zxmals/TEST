...
<body class="gray-bg" onload="DoCheck()">
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>获奖称号:</label> <input id="upinfscope" type="text"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATteachAchievementRewardLevelset!updateEntity",
					{"entity.rewardId":$('#upinfID').val().trim(),
					 "entity.rewardName":$('#upinfscope').val().trim(),
					 "entity.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATteachAchievementRewardLevelset!entityList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATteachAchievementRewardLevelset!deleteEntity",
							{"entity.rewardId":row[0].cells[0].innerHTML,
							 "entity.rewardName":row[0].cells[1].innerHTML,
