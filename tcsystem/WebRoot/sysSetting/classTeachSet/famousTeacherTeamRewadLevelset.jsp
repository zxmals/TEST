...
<body class="gray-bg" onload="DoCheck()">
						...
						<div class="example">
							<form method="post" name="f">
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${famousTeacherTeamRewadLevelList }">
											<tr>
												<td>${ebj.teachRewardLevelId }</td>
												<td>${ebj.teachRewardLevelName}</td>
												<td>${ebj.score }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>教学团队等级:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATfamousTeacherTeamRewadLevelset!addFamousTeacherTeamRewadLevel" method="post">
								<div class="form-group">
									<label>教学团队等级:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="famousTeacherTeamRewadLevel.teachRewardLevelName"
										value="">
								</div>
								<div class="form-group">
									<label>系数:</label> <input id="addinf2" type="text"
										class="form-control nullcheck"
										name="famousTeacherTeamRewadLevel.score" value="">
								</div>
							</form>
							<div>
								<button type="button"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATfamousTeacherTeamRewadLevelset!updatefamousTeacherTeamRewadLevel",
					{"famousTeacherTeamRewadLevel.teachRewardLevelId":$('#upinfID').val().trim(),
					 "famousTeacherTeamRewadLevel.teachRewardLevelName":$('#upinfscope').val().trim(),
					 "famousTeacherTeamRewadLevel.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATfamousTeacherTeamRewadLevelset!famousTeacherTeamRewadLevelList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATfamousTeacherTeamRewadLevelset!deletefamousTeacherTeamRewadLevel",
							{"famousTeacherTeamRewadLevel.teachRewardLevelId":row[0].cells[0].innerHTML,
							 "famousTeacherTeamRewadLevel.teachRewardLevelName":row[0].cells[1].innerHTML,
							 "famousTeacherTeamRewadLevel.score":row[0].cells[2].innerHTML},
							function(data,status){
								if(status=="success"){
									if(data.trim()=="succ"){
