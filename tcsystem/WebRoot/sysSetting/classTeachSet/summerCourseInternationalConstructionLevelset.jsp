...
<body class="gray-bg" onload="DoCheck()">
								...
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${entityList }">
											<tr>
												<td>${ebj.projectLevelId }</td>
												<td>${ebj.projectLevel}</td>
												<td>${ebj.score }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>课程等级:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATsummerCourseInternationalConstructionLevelset!addEntity" method="post">
								<div class="form-group">
									<label>课程等级:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="entity.projectLevel"
										value="">
								</div>
								<div class="form-group">
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATsummerCourseInternationalConstructionLevelset!updateEntity",
					{"entity.projectLevelId":$('#upinfID').val().trim(),
					 "entity.projectLevel":$('#upinfscope').val().trim(),
					 "entity.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATsummerCourseInternationalConstructionLevelset!entityList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATsummerCourseInternationalConstructionLevelset!deleteEntity",
							{"entity.projectLevelId":row[0].cells[0].innerHTML,
							 "entity.projectLevel":row[0].cells[1].innerHTML,
