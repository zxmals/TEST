...
<body class="gray-bg" onload="DoCheck()">
								...
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${entityList }">
											<tr>
												<td>${ebj.thesisEvaluationLevelId }</td>
												<td>${ebj.thesisEvaluationLevel}</td>
												<td>${ebj.ratio }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>评价等级:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATpracticeInnovationGuideGraduationThesisGuideEvalutionset!addEntity" method="post">
								<div class="form-group">
									<label>评价等级:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="entity.thesisEvaluationLevel"
										value="">
								</div>
								<div class="form-group">
									<label>系数:</label> <input id="addinf2" type="text"
										class="form-control nullcheck"
										name="entity.ratio" value="">
								</div>
							</form>
							<div>
								<button type="button"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){//$('#upinfscope').val().trim()!=""
			$.post("ATpracticeInnovationGuideGraduationThesisGuideEvalutionset!updateEntity",
					{"entity.thesisEvaluationLevelId":$('#upinfID').val().trim(),
					 "entity.thesisEvaluationLevel":$('#upinfscope').val().trim(),
					 "entity.ratio":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATpracticeInnovationGuideGraduationThesisGuideEvalutionset!entityList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATpracticeInnovationGuideGraduationThesisGuideEvalutionset!deleteEntity",
							{"entity.thesisEvaluationLevelId":row[0].cells[0].innerHTML,
							 "entity.thesisEvaluationLevel":row[0].cells[1].innerHTML,
							 "entity.ratio":row[0].cells[2].innerHTML},
							function(data,status){
								if(status=="success"){
									if(data.trim()=="succ"){
