...
<body class="gray-bg" onload="DoCheck()">
								...
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${entityList }">
											<tr>
												<td>${ebj.competitionTypeId }</td>
												<td>${ebj.competitionType}</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>竞赛等级:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATstudentCompetitionGuidanceCompetitionTypeset!addEntity" method="post">
								<div class="form-group">
									<label>竞赛等级:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="entity.competitionType"
										value="">
								</div>
							</form>
							<div>
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){//$('#upinfscope').val().trim()!=""
			$.post("ATstudentCompetitionGuidanceCompetitionTypeset!updateEntity",
					{"entity.competitionTypeId":$('#upinfID').val().trim(),
					 "entity.competitionType":$('#upinfscope').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								alert("更新成功");
								window.location.replace("ATstudentCompetitionGuidanceCompetitionTypeset!entityList");
							}else{
								alert("操作失败");
							}
						}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATstudentCompetitionGuidanceCompetitionTypeset!deleteEntity",
							{"entity.competitionTypeId":row[0].cells[0].innerHTML,
							 "entity.competitionType":row[0].cells[1].innerHTML},
							function(data,status){
								if(status=="success"){
									if(data.trim()=="succ"){
