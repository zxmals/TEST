...
<body class="gray-bg" onload="DoCheck()">
						...
						<div class="example">
							<form method="post" name="f">
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${teachingAbilityImproveLevelList}">
											<tr>
												<td>${ebj.improveLevelId}</td>
												<td>${ebj.improveLevel}</td>
												<td>${ebj.ratio }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>教学能力提升水平:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATteachingAbilityImproveLevelset!addTeachingAbilityImproveLevel" method="post">
								<div class="form-group">
									<label>教学能力提升水平:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="teachingAbilityImproveLevel.improveLevel"
										value="">
								</div>
								<div class="form-group">
									<label>系数:</label> <input id="addinf2" type="text"
										class="form-control nullcheck"
										name="teachingAbilityImproveLevel.ratio" value="">
								</div>
							</form>
							<div>
								<button type="button"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATteachingAbilityImproveLevelset!updateTeachingAbilityImproveLevel",
					{"teachingAbilityImproveLevel.improveLevelId":$('#upinfID').val().trim(),
					 "teachingAbilityImproveLevel.improveLevel":$('#upinfscope').val().trim(),
					 "teachingAbilityImproveLevel.ratio":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATteachingAbilityImproveLevelset!teachingAbilityImproveLevelList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
					$.post("ATteachingAbilityImproveLevelset!deleteTeachingAbilityImproveLevel",
							{"teachingAbilityImproveLevel.improveLevelId":row[0].cells[0].innerHTML,
							 "teachingAbilityImproveLevel.improveLevel":row[0].cells[1].innerHTML,
							 "teachingAbilityImproveLevel.score":row[0].cells[2].innerHTML},
							function(data,status){
								if(status=="success"){
									if(data.trim()=="succ"){
