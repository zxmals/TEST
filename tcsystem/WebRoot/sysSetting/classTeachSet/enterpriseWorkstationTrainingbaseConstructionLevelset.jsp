...
<body class="gray-bg" onload="DoCheck()">
								...
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${entityList }">
											<tr>
												<td>${ebj.trainingConstruLevelId }</td>
												<td>${ebj.trainingConstruLevel}</td>
												<td>${ebj.score }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>培训基地等级:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATenterpriseWorkstationTrainingbaseConstructionLevelset!addEntity" method="post">
								<div class="form-group">
									<label>培训基地等级:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="entity.trainingConstruLevel"
										value="">
								</div>
								<div class="form-group">
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATenterpriseWorkstationTrainingbaseConstructionLevelset!updateEntity",
					{"entity.trainingConstruLevelId":$('#upinfID').val().trim(),
					 "entity.trainingConstruLevel":$('#upinfscope').val().trim(),
					 "entity.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATenterpriseWorkstationTrainingbaseConstructionLevelset!entityList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATenterpriseWorkstationTrainingbaseConstructionLevelset!deleteEntity",
							{"entity.trainingConstruLevelId":row[0].cells[0].innerHTML,
							 "entity.trainingConstruLevel":row[0].cells[1].innerHTML,
