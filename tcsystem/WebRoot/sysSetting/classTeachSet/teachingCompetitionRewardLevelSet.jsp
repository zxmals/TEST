...
<body class="gray-bg" onload="DoCheck()">
						...
						<div class="example">
							<form method="post" name="f">
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${teachingCompetitionRewardLevelList}">
											<tr>
												<td>${ebj.competRewardLevelId}</td>
												<td>${ebj.competRewardLevel}</td>
												<td>${ebj.score }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>竞赛获奖等级:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATteachingCompetitionRewardLevelset!addTeachingCompetitionRewardLevel" method="post">
								<div class="form-group">
									<label>竞赛获奖等级:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="teachingCompetitionRewardLevel.competRewardLevel"
										value="">
								</div>
								<div class="form-group">
									<label>系数:</label> <input id="addinf2" type="text"
										class="form-control nullcheck"
										name="teachingCompetitionRewardLevel.score" value="">
								</div>
							</form>
							<div>
								<button type="button"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATteachingCompetitionRewardLevelset!updateTeachingCompetitionRewardLevel",
					{"teachingCompetitionRewardLevel.competRewardLevelId":$('#upinfID').val().trim(),
					 "teachingCompetitionRewardLevel.competRewardLevel":$('#upinfscope').val().trim(),
					 "teachingCompetitionRewardLevel.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATteachingCompetitionRewardLevelset!teachingCompetitionRewardLevelList");
							}else{
	...
	$('.delinf').click(function() {
		 ...
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATteachingCompetitionRewardLevelset!deleteTeachingCompetitionRewardLevel",
							{"teachingCompetitionRewardLevel.competRewardLevelId":row[0].cells[0].innerHTML,
							 "teachingCompetitionRewardLevel.competLevel":row[0].cells[1].innerHTML,
							 "teachingCompetitionRewardLevel.score":row[0].cells[2].innerHTML},
							function(data,status){
								if(status=="success"){
									if(data.trim()=="succ"){
