...
<body class="gray-bg" onload="DoCheck()">
						...
						<div class="example">
							<form method="post" name="f">
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${teachEvaluationList }">
											<tr>
												<td>${ebj.evaluationId}</td>
												<td>${ebj.evaluResult}</td>
												<td>${ebj.ratio }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>评价结果:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATteachEvaluationset!addTeachEvaluation" method="post">
								<div class="form-group">
									<label>评价结果:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="teachEvaluation.evaluResult"
										value="">
								</div>
								<div class="form-group">
									<label>系数:</label> <input id="addinf2" type="text"
										class="form-control nullcheck"
										name="teachEvaluation.ratio" value="">
								</div>
							</form>
							<div>
								<button type="button"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){
			$.post("ATteachEvaluationset!updateTeachEvaluation",
					{"teachEvaluation.evaluationId":$('#upinfID').val().trim(),
					 "teachEvaluation.evaluResult":$('#upinfscope').val().trim(),
					 "teachEvaluation.ratio":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATteachEvaluationset!teachEvaluation");
							}else{
	...
	$('.delinf').click(function() {
		 ...
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATteachEvaluationset!deleteTeachEvaluation",
							{"teachEvaluation.evaluationId":row[0].cells[0].innerHTML,
							 "teachEvaluation.evaluResult":row[0].cells[1].innerHTML,
							 "teachEvaluation.ratio":row[0].cells[2].innerHTML},
							function(data,status){
								if(status=="success"){
									if(data.trim()=="succ"){
