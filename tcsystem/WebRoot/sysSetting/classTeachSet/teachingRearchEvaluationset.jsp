...
<body class="gray-bg" onload="DoCheck()">
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>成果:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATteachRearchEvaluationset!addEntity" method="post">
								<div class="form-group">
									<label>成果:</label> <input id="addinf1" type="text"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){//$('#upinf').val().trim()!=""
			$.post("ATteachRearchEvaluationset!updateEntity",
					{"entity.evaluationId":$('#upinfID').val().trim(),
					 "entity.reaults":$('#upinfscope').val().trim(),
					 "entity.ratio":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATteachRearchEvaluationset!entityList");
							}else{
	...
	$('.delinf').click(function() {
		var row = $(this).parent().parent();
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATteachRearchEvaluationset!deleteEntity",
							{"entity.evaluationId":row[0].cells[0].innerHTML,
							 "entity.reaults":row[0].cells[1].innerHTML,
