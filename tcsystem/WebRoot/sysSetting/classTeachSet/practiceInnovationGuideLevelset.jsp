...
<body class="gray-bg" onload="DoCheck()">
								...
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${entityList }">
											<tr>
												<td>${ebj.innovationGuideLevelId }</td>
												<td>${ebj.innovatIonguideLevel}</td>
												<td>${ebj.score }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>创新项目等级:</label> <input id="upinfscope" type="text"
					...
					<div class="modal-body">
						<div class="row">
							...
							<form role="form" id="onlyForm" name="adds"
								action="ATpracticeInnovationGuideLevelset!addEntity" method="post">
								<div class="form-group">
									<label>创新项目等级:</label> <input id="addinf1" type="text"
										class="form-control nullcheck" name="entity.innovatIonguideLevel"
										value="">
								</div>
								<div class="form-group">
									<label>分数:</label> <input id="addinf2" type="text"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){//$('#upinfscope').val().trim()!=""
			$.post("ATpracticeInnovationGuideLevelset!updateEntity",
					{"entity.innovationGuideLevelId":$('#upinfID').val().trim(),
					 "entity.innovatIonguideLevel":$('#upinfscope').val().trim(),
					 "entity.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATpracticeInnovationGuideLevelset!entityList");
							}else{
	...
	$('.delinf').click(function() {
		 ...
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATpracticeInnovationGuideLevelset!deleteEntity",
							{"entity.innovationGuideLevelId":row[0].cells[0].innerHTML,
							 "entity.innovatIonguideLevel":row[0].cells[1].innerHTML,
