...
<html lang="zh-CN">
  ...
  <base target="_self"> 
    ...
    <title>InviteExpertSpeechSet--Expert-Type --Set</title>
...
<body class="gray-bg"  onload="DoCheck()">
            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>邀请专家讲学设置 --专家类别设置<small></small></h5>
                    ...
                    <div class="">
                       ...
                       <table  id="tb" class="table table-striped table-bordered table-hover dataTables-example">
                            ...
                            <tbody>
								<c:forEach var="ebj"  items="${expertTypeli }">
								<tr>
									<td>${ebj.expertTypeId }</td>
									<td>${ebj.expertTypeName }</td>
   ...
   <div id="update" class="modal fade" aria-hidden="true"tabindex="-1" role="dialog"     aria-labelledby="myModalLabel">
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="row">
                            ...
                            <form role="form" id="onlyForm" name="adds"action="ATInviteExpertSpeechExpertTypeset!addExpertType" method="post">                            
                                <div class="form-group">                                
                                    <label>专家类型:</label>
                                    <input id="addinf" type="text"  class="form-control nullcheck" name=expertTyp.expertTypeName   value="">
                                </div>                                                           
                            </form>
                                <div>
                                    <button type="button"   class="btn btn-outline btn-primary pull-right m-t-n-xs" data-dismiss="modal">关闭</button>
    ...
    $('#subupdate').click(function() {
		if($('#upinf').val().trim()!=""){
			$.post("ATInviteExpertSpeechExpertTypeset!updateExpertType",
					{"expertTyp.expertTypeId":$('#upinfID').val().trim(),
					 "expertTyp.expertTypeName":$('#upinf').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								alert("更新成功");
								window.location.replace("ATInviteExpertSpeechExpertTypeset!getExpertTypeINF");
							}else{
								alert("操作失败");
							}
						}else{
	...
	$('.delinf').click(function() {
		...
		if(x){
			$.post("ATInviteExpertSpeechExpertTypeset!deleteExpertType",
					{"expertTyp.expertTypeId":row[0].cells[0].innerHTML,
					 "expertTyp.expertTypeName":row[0].cells[1].innerHTML},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								alert("删除成功");
