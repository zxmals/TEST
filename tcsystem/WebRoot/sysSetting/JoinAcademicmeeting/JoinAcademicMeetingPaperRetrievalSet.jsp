...
<body class="gray-bg"  onload="DoCheck()">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="">
                       ...
                       <table  id="tb" class="table table-striped table-bordered table-hover dataTables-example">
                            ...
                            <tbody>
								<c:forEach var="ebj"  items="${meetingpaperretrievalli }">
								<tr>
									<td>${ebj.prconditionId }</td>
									<td>${ebj.prcondition }</td>
   ...
   <div id="update" class="modal fade" aria-hidden="true"tabindex="-1" role="dialog"     aria-labelledby="myModalLabel">
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="row">
                            ...
                            <form role="form" id="onlyForm" name="adds"action="ATJoinAcademicMeetingPaperRetrievalset!addPaperRetrievalCondition" method="post">                            
                                <div class="form-group">                                
                                    <label>会议地点:</label>
                                    <input id="addinf" type="text"  class="form-control nullcheck" name=paperretrieval.prcondition   value="">
                                </div>                                                           
                            </form>
                                <div>
                                    <button type="button"   class="btn btn-outline btn-primary pull-right m-t-n-xs" data-dismiss="modal">关闭</button>
    ...
    $('#subupdate').click(function() {
		if($('#upinf').val().trim()!=""){
			$.post("ATJoinAcademicMeetingPaperRetrievalset!updatePaperRetrievalCondition",
					{"paperretrieval.prconditionId":$('#upinfID').val().trim(),
					 "paperretrieval.prcondition":$('#upinf').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								alert("更新成功");
								window.location.replace("ATJoinAcademicMeetingPaperRetrievalset!getPaperRetrievalConditionINF");
							}else{
								alert("操作失败");
							}
						}else{
	...
	$('.delinf').click(function() {
		...
		if(x){
		$.post("ATJoinAcademicMeetingPaperRetrievalset!deletePaperRetrievalCondition",
					{"paperretrieval.prconditionId":row[0].cells[0].innerHTML,
					 "paperretrieval.prcondition":row[0].cells[1].innerHTML},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								alert("删除成功");
