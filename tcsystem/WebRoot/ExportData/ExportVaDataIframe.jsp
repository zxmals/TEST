				...
				<tbody>
					<c:forEach var="vaact" items="${vaactList }">
						<tr>
							...
							<td>${vaact.actDate  }</td>
							<c:if test="${vaact.vacollectiveAct.actType==1 }"><td>规定性活动</td></c:if>
							<c:if test="${vaact.vacollectiveAct.actType==2 }"><td>选择性活动</td></c:if>
							<c:if test="${vaact.vacollectiveAct.actType==3 }"><td>其他活动</td></c:if>
                                            
							<td><button class="button_set" type="submit" id="vaactexports_${vaact.actPubId }" data-backdrop="true" onclick="exportAct('${vaact.actPubId}')">导出该活动</button></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
