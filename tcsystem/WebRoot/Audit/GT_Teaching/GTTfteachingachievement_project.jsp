...
<body style="padding-top:0px;margin-top:0px;">
	...
	<form
		action="GTTFteachingachievement_projectAudit11!getAllRecord_project"
...
<!-- 					onClick="WdatePicker()" value="${sessionScope.foredate_TFTA }" -->
<!-- 					name="foredate_TFTA"  />到:<input type="text" -->
<!-- 					id="date2" onClick="WdatePicker()" class="Wdate" id="afterdate" -->
<!-- 					value="${sessionScope.afterdate_TFTA }" name="afterdate_TFTA" -->
<!-- 					 /> &nbsp;&nbsp;<input type="submit" id="datep" -->
<!-- 					value="查询" title="点击查询"> -->
<!-- 			</div> -->
<!-- 		</div> -->
		<h3 style="padding:0px;margin-left: 10px;">教学成果奖项目审核</h3>
		<hr>
		...
		<span style="color:green;">${sessionScope.teacherDepartment }</span>
		 ...
		 <span>每页显示： <select
			name="pageSize_TFTA" id="pageSizeSelection">
		...
		<span>&nbsp;&nbsp;&nbsp;&nbsp;
			<select name="term_TFTA.termId" id="termSelection">
		...
		<span>&nbsp;&nbsp;&nbsp;&nbsp; 审核状态： <select
			name="checkOutStatus_TFTA" id="checkoutStatus">
	...
	<form name="Audit" action="" method="post">
		<table
			...
			<tr>
				...
				<td>登记负责人Id</td>		
				<td>登记负责人</td>
<!-- 				<c:if test="${sessionScope.checkOutStatus_TFTA=='0' }"> -->
...
<!-- 				</c:if> -->
				...
				<c:if test="${sessionScope.checkOutStatus_TFTA=='1' }">
					<td><font color="blue">通过</td>
				</c:if>
				<c:if test="${sessionScope.checkOutStatus_TFTA=='2' }">
					<td><font color="red">未通过</td>
				</c:if>
				<c:if test="${sessionScope.checkOutStatus_TFTA=='4' }">
					<td><font color="blue">审核状态</font></td>
				</c:if>
				<c:if test="${sessionScope.checkOutStatus_TFTA=='5' }">
					<td><font color="orange">待完善</font></td>
				</c:if>
			</tr>
			<c:forEach var="entity"
				items="${TFteachingAchievement }">
				<tr>
					<!-- 教学研究项目Id -->
					...
					<!-- 是否合作-->
					<td>${entity.cooperator }
						<c:if test="${entity.cooperator=='1' }">是</c:if>
						<c:if test="${entity.cooperator=='0' }">否</c:if>
					</td>
					<!-- 获奖级别 -->
					<td>${entity.tfteachingAchievementRewardLevel.rewardName }</td>
					<!-- 项目总分 -->
...
<!-- 					</c:if> -->
	...
	<div id="dividePageDev" style="height: 30px;">
		<span style="font-size:12px;color:#727272;"> 当前是第<font
			style=" color:blue; font-weight: bold;">${pageIndex }/${sessionScope.pageCount_TFTA }</font>页
		</span> <span> <c:if test="${pageIndex>1}">
				<a
					href="GTTFteachingachievement_projectAudit11!getAllRecord_projectAfterDivide?pageIndex=${pageIndex-1 }">上一页</a>
			</c:if>
		</span>

		<c:forEach begin="${pageIndex }" end="${pageIndex+4 }" var="index"
			step="1">
			<c:if test="${index<=pageCount_TFTA }">
				<span> <a
					href="GTTFteachingachievement_projectAudit11!getAllRecord_projectAfterDivide?pageIndex=${index }">${index }</a>
				</span>
			</c:if>
		</c:forEach>
		<span> <c:if test="${pageIndex<pageCount_TFTA }">
				<a
					href="GTTFteachingachievement_projectAudit11!getAllRecord_projectAfterDivide?pageIndex=${pageIndex+1 }">下一页</a>
			</c:if>
		</span> <span> 共<font style="color:blue;">${sessionScope.pageCount_TFTA }</font>页
		</span> <span> 共<font style="color:blue;">${sessionScope.recordNumber_TFTA }</font>条记录
		</span>
	</div>
	<c:if test="${sessionScope.checkOutStatus_TFTA=='0'}">
	...
	<script type="text/javascript">
		$().ready(function(){
			$("#pageSizeSelection option[value='${sessionScope.pageSize_TFTA}']").attr("selected",true);
			$("#checkoutStatus option[value='${sessionScope.checkOutStatus_TFTA}']").attr("selected",true);
			$("#termSelection option[value='${sessionScope.term_TFTA.termId}']").attr("selected",true);
		});
		$("#doCheckout").click(function(){
			submitAudit("GTTFteachingachievement_projectAudit11!doCheckOutTask",
					"GTTFteachingachievement_projectAudit11!getAllRecord_project");
		});
