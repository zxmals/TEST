...
<body style="padding-top:0px;margin-top:0px;">
	...
	<form
		action="GTTFteachingtextbook_projectAudit11!getAllRecord_project"
...
<!-- 			</div> -->
<!-- 		</div> -->
		<h3 style="padding:0px;margin-left: 10px;">教材建设项目审核</h3>
		<hr>
...
<!-- 				</c:if> -->
			...
			<c:forEach var="entity"
				...
				<tr>
					...
					<!-- 教材等级 -->
					<td>${entity.tftextbookConstructionTblevel.tblevel }</td>
					<!-- 项目总分 -->
...
<!-- 					</c:if> -->
					<c:if test="${sessionScope.checkOutStatus_TFTR=='0' }">
						<td class="c1">通过&nbsp;<input type="checkbox"
							name="chooseWhichToAudit" value="${entity.bookId }"
							class="check1" /></td>
						<td class="c2">不通过<input
							value="${entity.bookId }" type="checkbox"
	...
	<div id="dividePageDev" style="height: 30px;">
		<span style="font-size:12px;color:#727272;"> 当前是第<font
			style=" color:blue; font-weight: bold;">${pageIndex }/${sessionScope.pageCount_TFTR }</font>页
				...
				<a
					href="GTTFteachingtextbook_projectAudit11!getAllRecord_projectAfterDivide?pageIndex=${pageIndex-1 }">上一页</a>
			</c:if>
		</span>
		...
		<c:forEach begin="${pageIndex }" end="${pageIndex+4 }" var="index"
			...
			<c:if test="${index<=pageCount_TFTR }">
				<span> <a
					href="GTTFteachingtextbook_projectAudit11!getAllRecord_projectAfterDivide?pageIndex=${index }">${index }</a>
				</span>
			</c:if>
		</c:forEach>
		<span> <c:if test="${pageIndex<pageCount_TFTR }">
				<a
					href="GTTFteachingtextbook_projectAudit11!getAllRecord_projectAfterDivide?pageIndex=${pageIndex+1 }">下一页</a>
			</c:if>
	...
	<script type="text/javascript">
		...
		$("#doCheckout").click(function(){
			submitAudit("GTTFteachingtextbook_projectAudit11!doCheckOutTask",
					"GTTFteachingtextbook_projectAudit11!getAllRecord_project");
		});
