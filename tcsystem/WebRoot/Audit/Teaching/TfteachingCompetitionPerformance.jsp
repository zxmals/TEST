...
<body style="padding-top:0px;margin-top:0px;">
	...
	<form
		...
		<span style="margin-left:10px;">所在系：&nbsp;&nbsp;&nbsp;&nbsp;</span> <span>
			<select name="department_TCP.departmentId"
		...
		<span>
			学期：
			<select id="termSelection" name="termId_TCP">
		...
		<span>每页显示： <select
			name="pageSize_TCP" id="pageSizeSelection">
			...
			name="checkOutStatus_TCP" id="checkoutStatus">
	...
	<form name="Audit" action="" method="post">
		<table
			...
			<tr>
				...
				<td>教师姓名</td>
				<td>最终分数</td>
				<c:if test="${sessionScope.checkOutStatus_TCP=='1' }">
					<td>全通过&nbsp;<input type="checkbox" name="" id="allAudit" /></td>
					<td>全不通过<input type="checkbox" id="allNotAudit"></td>
				</c:if>
				<c:if test="${sessionScope.checkOutStatus_TCP=='3' }">
					<td><font color="blue">通过</td>
				</c:if>
				<c:if test="${sessionScope.checkOutStatus_TCP=='2' }">
					<td><font color="red">未通过</td>
				</c:if>
				<c:if test="${sessionScope.checkOutStatus_TCP=='4' }">
					<td><font color="blue">审核记录</td>
				</c:if>

			</tr>
			<c:forEach var="TfteachingCompetitionPer"
				items="${TfteachingCompetitionPerfList }">
				<tr>
					<!-- 教学竞赛编号 -->
					<td>${TfteachingCompetitionPer.competitionId }</td>
					<!-- 竞赛名称-->
					<td>${TfteachingCompetitionPer.competitionName }</td>
					<!-- 竞赛获奖等级-->
					<td>${TfteachingCompetitionPer.tfteachingCompetitionRewardLevel.competRewardLevel }</td>
					<!-- 教师编号 -->
					<td>${TfteachingCompetitionPer.teacher.teacherId }</td>
					<!-- 教师姓名 -->
					<td>${TfteachingCompetitionPer.teacher.teacherName }</td>
					<!-- 最终分数 -->
					<td>${TfteachingCompetitionPer.finalScore }</td>
					<c:if test="${sessionScope.checkOutStatus_TCP=='1' }">
						<td class="c1">通过&nbsp;<input type="checkbox" name="chooseWhichToAudit"
							value="${TfteachingCompetitionPer.upid}"   class="check1"/></td>
						<td class="c2">不通过<input value="${TfteachingCompetitionPer.upid}" type="checkbox"
					...
					<c:if test="${sessionScope.checkOutStatus_TCP=='4' }">
						<c:if test="${TfteachingCompetitionPer.checkOut== '1' }">
							<td><font color="orange" size="2">待审核</font></td>
						</c:if>
						<c:if test="${TfteachingCompetitionPer.checkOut== '3' }">
							<td><font color="green" size="2">审核通过</font></td>
							</c:if>
						<c:if test="${TfteachingCompetitionPer.checkOut== '2' }">
	...
	<script type="text/javascript">
		$().ready(function(){
			$("#pageSizeSelection option[value='${sessionScope.pageSize_TCP}']").attr("selected",true);
			$("#departmentSelection option[value='${sessionScope.department_TCP.departmentId}']").attr("selected",true);
			$("#checkoutStatus option[value='${sessionScope.checkOutStatus_TCP}']").attr("selected",true);
