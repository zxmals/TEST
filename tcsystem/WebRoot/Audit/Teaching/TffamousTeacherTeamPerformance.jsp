...
<body style="padding-top:0px;margin-top:0px;">
	...
	<form
		action="ATTffamousTeacherTeamPerformanceAudit!getTfFamousTeacherTermPerfList"
	...
	<form name="Audit" action="" method="post">
		<table
			...
			<tr>
				...
				<c:if test="${sessionScope.checkOutStatus_FTT=='3' }">
					<td><font color="green">管理员审核通过</td>
				</c:if>				
				<c:if test="${sessionScope.checkOutStatus_FTT=='4' }">
			...
			<c:forEach var="TfFamousTeacherTeamPerf"
				...
				<tr>
					...
					<c:if test="${sessionScope.checkOutStatus_FTT=='4' }">
						<c:if test="${TfFamousTeacherTeamPerf.checkOut=='0' }">
							<td><font color="orange" >等待系主任审核</td>
						</c:if>
						<c:if test="${TfFamousTeacherTeamPerf.checkOut=='1' }">
							<td><font color="blue" >待管理员审核</td>
						</c:if>
						<c:if test="${TfFamousTeacherTeamPerf.checkOut=='2' }">
							<td><font color="red">未通过</td>
						</c:if>				
						<c:if test="${TfFamousTeacherTeamPerf.checkOut=='3' }">
							<td><font color="green">管理员审核通过</td>
						</c:if>				
					</c:if>
				</tr>
	...
	<div id="dividePageDev" style="height: 30px;">
		<span style="font-size:12px;color:#727272;"> 当前是第<font
			style=" color:blue; font-weight: bold;">${pageIndex }/${sessionScope.pageCount_FTT }</font>页
				...
				<a
					href="ATTffamousTeacherTeamPerformanceAudit!getTfFamousTeacherTermPerfList?isDivided=true&pageIndex=${pageIndex-1 }">上一页</a>
			</c:if>
		</span>
		...
		<c:forEach begin="${pageIndex }" end="${pageIndex+4 }" var="index"
			...
			<c:if test="${index<=pageCount_FTT }">
				<span> <a
					href="ATTffamousTeacherTeamPerformanceAudit!getTfFamousTeacherTermPerfList?isDivided=true&pageIndex=${index }">${index }</a>
				</span>
			</c:if>
		</c:forEach>
		<span> <c:if test="${pageIndex<pageCount_FTT }">
				<a
					href="ATTffamousTeacherTeamPerformanceAudit!getTfFamousTeacherTermPerfList?isDivided=true&pageIndex=${pageIndex+1 }">下一页</a>
			</c:if>
	...
	<script type="text/javascript">
		...
		$("#doCheckout").click(function(){
			submitAudit("ATTffamousTeacherTeamPerformanceAudit!doCheckOutTask",
					"ATTffamousTeacherTeamPerformanceAudit!getTfFamousTeacherTermPerfList");
		});
	</script>
</body>
</html>
