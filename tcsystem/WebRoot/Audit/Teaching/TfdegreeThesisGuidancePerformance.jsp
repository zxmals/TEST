	...
	<form name="Audit" action="" method="post">
		<table
			...
			<tr>
				...
				<c:if test="${sessionScope.checkOutStatus_DTG=='3' }">
					<td><font color="blue">管理员审核通过</font></td>
				</c:if>
				<c:if test="${sessionScope.checkOutStatus_DTG=='4' }">
			...
			<c:forEach var="TfDegreeThesisGuidancePer"
				...
				<tr>
					...
					<c:if test="${sessionScope.checkOutStatus_DTG=='0' }">
						<td><font color="orange">等待系主任审核</font></td>
					</c:if>
					<c:if test="${sessionScope.checkOutStatus_DTG=='1' }">
					...
					<c:if test="${sessionScope.checkOutStatus_DTG=='4' }">
						<c:if test="${TfDegreeThesisGuidancePer.checkOut== '1' }">
							<td><font color="orange" size="2">待审核</font></td>
						</c:if>
						<c:if test="${TfDegreeThesisGuidancePer.checkOut== '3' }">
							<td><font color="green" size="2">审核通过</font></td>
							</c:if>
						<c:if test="${TfDegreeThesisGuidancePer.checkOut== '2' }">
