...
<body style="padding-top:0px;margin-top:0px;">
	...
	<form
		...
		<span style="margin-left:10px;">所在系：&nbsp;&nbsp;&nbsp;&nbsp;</span> <span>
			<select name="department_EWTB.departmentId"
		...
		<span>
			学期：
			<select id="termSelection" name="termId_EWTB">
		...
		<span>每页显示： <select
			name="pageSize_EWTB" id="pageSizeSelection">
			...
			name="checkOutStatus_EWTB" id="checkoutStatus">
	...
	<form name="Audit" action="" method="post">
		<table
			...
			<tr>
				...
				<td>项目总分</td>
				<td>最终分数</td>
				<c:if test="${sessionScope.checkOutStatus_EWTB=='1' }">
					<td>全通过&nbsp;<input type="checkbox" name="" id="allAudit" /></td>
					<td>全不通过<input type="checkbox" id="allNotAudit"></td>
				</c:if>
				<c:if test="${sessionScope.checkOutStatus_EWTB=='0' }">
					<td><font color="blue">待系主任审核</td>
				</c:if>				
				<c:if test="${sessionScope.checkOutStatus_EWTB=='2' }">
					<td><font color="red">未通过</td>
				</c:if>				
				<c:if test="${sessionScope.checkOutStatus_EWTB=='3' }">
					<td><font color="green">管理员审核通过</td>
				</c:if>				
				<c:if test="${sessionScope.checkOutStatus_EWTB=='4' }">
					<td><font color="blue">查看全部记录</td>
				</c:if>	

			</tr>
			<c:forEach var="Tf_EN_WTB_CONS_PERF"
				items="${Tf_EN_WTB_CONS_PERF_List }">
				<tr>
					<!-- 项目编号-->
					<td>${Tf_EN_WTB_CONS_PERF.tfenterpriseWorkstationTrainingBaseConstructionProject.projectId }</td>
					<!-- 项目名称-->
					<td>${Tf_EN_WTB_CONS_PERF.tfenterpriseWorkstationTrainingBaseConstructionProject.projectName }</td>
					<!-- 项目类型 -->
					<td>${Tf_EN_WTB_CONS_PERF.tfenterpriseWorkstationTrainingBaseConstructionProject.tfenterpriseWorkstationTrainingbaseConstructionLevel.trainingConstruLevel }</td>
					<!-- 本人承担任务 -->
					<td>${Tf_EN_WTB_CONS_PERF.selfUndertakeTask.undertakeTaskName }</td>
					<!-- 单位数量 -->
					<td>${Tf_EN_WTB_CONS_PERF.tfenterpriseWorkstationTrainingBaseConstructionProject.quantityUnit }</td>
					<!-- 教师编号 -->
					<td>${Tf_EN_WTB_CONS_PERF.teacher.teacherId }</td>
					<!-- 教师姓名 -->
					<td>${Tf_EN_WTB_CONS_PERF.teacher.teacherName }</td>
					<!-- 项目总分 -->
					<td>${Tf_EN_WTB_CONS_PERF.tfenterpriseWorkstationTrainingBaseConstructionProject.projectSumScore }</td>
					<!-- 最终分数 -->
					<td>${Tf_EN_WTB_CONS_PERF.singleScore }</td>
					<c:if test="${sessionScope.checkOutStatus_EWTB=='1' }">
						<td class="c1">通过&nbsp;<input type="checkbox" name="chooseWhichToAudit"
							value="${Tf_EN_WTB_CONS_PERF.upid}"  class="check1"/></td>
						<td class="c2">不通过<input value="${Tf_EN_WTB_CONS_PERF.upid}" type="checkbox"
					...
					<c:if test="${sessionScope.checkOutStatus_EWTB=='4' }">
						<c:if test="${Tf_EN_WTB_CONS_PERF.checkOut=='0' }">
							<td><font color="orange" >等待系主任审核</td>
						</c:if>
						<c:if test="${Tf_EN_WTB_CONS_PERF.checkOut=='1' }">
							<td><font color="blue" >待管理员审核</td>
						</c:if>
						<c:if test="${Tf_EN_WTB_CONS_PERF.checkOut=='2' }">
							<td><font color="red">未通过</td>
						</c:if>				
						<c:if test="${Tf_EN_WTB_CONS_PERF.checkOut=='3' }">
							<td><font color="green">管理员审核通过</td>
						</c:if>				
					</c:if>
				</tr>
	...
	<script type="text/javascript">
		$().ready(function(){
			$("#pageSizeSelection option[value='${sessionScope.pageSize_EWTB}']").attr("selected",true);
			$("#departmentSelection option[value='${sessionScope.department_EWTB.departmentId}']").attr("selected",true);
			$("#checkoutStatus option[value='${sessionScope.checkOutStatus_EWTB}']").attr("selected",true);
