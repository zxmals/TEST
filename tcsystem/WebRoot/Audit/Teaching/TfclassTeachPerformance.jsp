...
<script type="text/javascript">
		...
		$("#doCheckout").click(function(){
			...
			swal({ 
				...
				function(isConfirm){
					if(isConfirm){
						$.post("TfclassTeachPerformanceAudit!doCheckOutTask",{checkOutIDs:IDs},function(data,status){
							if(status=="success"){
								if(data=="succ"){
									swal("审核成功！", "", "success");
									setTimeout(function(){
										window.location.replace("<%=basePath%>TfclassTeachPerformanceAudit!getTF_classTeachPerformanceList");
									},2000);
								}else{
									swal("审核失败！", "", "error");
								}
