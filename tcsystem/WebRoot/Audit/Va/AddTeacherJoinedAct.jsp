...
<body style="padding-top:0px;margin-top:0px;">
	...
	<form action="GTAddJoinedActAudit!getAddJoinedActList" method="post"
		name="pickdate" id="conditionForm">
		<div class="datepick" style="font-size:12px;">
			...
			<div>
				...
				从:<input type="text" class="Wdate" id="foredate"
					...
					name="foredate_CT" />到:<input type="text" onClick="WdatePicker()"
					class="Wdate" id="afterdate" value="${sessionScope.afterdate_CT }"
					name="afterdate_CT" /> &nbsp;&nbsp;<input type="submit"
					id="datep" value="查询" title="点击查询">
			</div>
		</div>
	...
	<form name="Audit" action="" method="post">
		<table
			...
			<c:forEach var="VaAddJoinedAct" items="${VaAddJoinedActList }">
				<tr>
					<!-- 公益活动编号 -->
					...
					<!-- 活动类型 -->
					<c:if
						test="${VaAddJoinedAct.id.vacollectiveActivitiesPublish.vacollectiveAct.actType==1 }">
						<td>规定性活动</td>
					</c:if>
					<c:if
						test="${VaAddJoinedAct.id.vacollectiveActivitiesPublish.vacollectiveAct.actType==2 }">
						<td>选择性活动</td>
					</c:if>
					<c:if
						test="${VaAddJoinedAct.id.vacollectiveActivitiesPublish.vacollectiveAct.actType==3 }">
						<td>其他活动</td>
					</c:if>
					<!-- 教师编号 -->
					...
					<c:if test="${sessionScope.checkOutStatus_CT=='0' }">
						<td class="c1">通过&nbsp;<input type="checkbox"
							name="chooseWhichToAudit"
							value="${VaAddJoinedAct.id.vacollectiveActivitiesPublish.actPubId }-${VaAddJoinedAct.id.teacher.teacherId }"
							class="check1" /></td>
