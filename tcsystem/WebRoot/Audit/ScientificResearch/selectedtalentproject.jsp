...
<body style="padding-top:0px;margin-top:0px;">
	...
	<form action="TeacherAndSelectedTalentProjectAudit!getTASTalentProjectInfo"
		...
		<span style="margin-left:10px;">研究所：&nbsp;&nbsp;&nbsp;&nbsp;</span> <span>
			<select name="researchLab_TAST.researchLabId" id="reserchLabSelection">
				<c:forEach var="researchLab" items="${researchLabList_TAST }">
	...
	<form name="Audit" action="" method="post">
		<table
			...
			<tr>
				...
				<c:if test="${sessionScope.checkOutStatus_TAST=='2' }">
					<td><font color="red">未通过</td>
				</c:if>

			</tr>
			<c:forEach var="TASTalentPro" items="${TAS_talentProList }">
				<tr>
					<!-- 人才工程编号 -->
					<td>${TASTalentPro.talentProject.talentProjectId }</td>
					<!-- 人才工程名称 -->
					<td>${TASTalentPro.talentProject.talentProjectName }</td>
					<!-- 教师编号 -->
					<td>${TASTalentPro.teacher.teacherId }</td>
					<!-- 教师姓名 -->
					<td>${TASTalentPro.teacher.teacherName }</td>
					<!-- 入选年份 -->
					<td>${TASTalentPro.tpselectedYear }</td>
					<!-- 最终分数 -->
		...
		<span style="font-size:12px;color:#727272;">
		 ...
		 <span> 
			<c:if
				test="${pageIndex>1}">
				<a
					href="TeacherAndSelectedTalentProjectAudit!getTASTalentProListAfterDivide?pageIndex=${pageIndex-1 }">上一页</a>
			</c:if>
		</span>

		<c:forEach begin="${pageIndex }" end="${pageIndex+4 }" var="index"
			step="1">
			<c:if test="${index<=pageCount_TAST }">
				<span> <a
					href="TeacherAndSelectedTalentProjectAudit!getTASTalentProListAfterDivide?pageIndex=${index }">${index }</a>
				</span>
			</c:if>
		</c:forEach>
		<span> <c:if test="${pageIndex<pageCount_TAST }">
				<a
					href="TeacherAndSelectedTalentProjectAudit!getTASTalentProListAfterDivide?pageIndex=${pageIndex+1 }">下一页</a>
			</c:if>
		</span> <span> 共<font style="color:blue;">${sessionScope.pageCount_TAST }</font>页
		</span> <span> 共<font style="color:blue;">${sessionScope.recordNumber_TAST }</font>条记录
		</span>
	</div>
	<c:if test="${sessionScope.checkOutStatus_TAST=='0'}">
...
<script type="text/javascript">
		...
		$("#doCheckout").click(function(){
			...
			if(window.confirm("您确认要提交审核吗？")){
				$.post("TeacherAndSelectedTalentProjectAudit!doCheckOutTask",{
					checkOutIDs:IDs
				},function(data,status){
					if(status=="success"){
						if(data=="succ"){
							window.alert("审核成功！");
							window.location.replace("<%=basePath%>TeacherAndSelectedTalentProjectAudit!getTASTalentProjectInfo");
						} else {
							window.alert("审核失败！");
						}
					}
