	...
	<form name="Audit" action="" method="post">
		<table
			...
			<tr>
				...
				<c:if test="${sessionScope.checkOutStatus_TAPA=='3' }">
					<td><font color="green">管理员审核通过</td>
				</c:if>				
				<c:if test="${sessionScope.checkOutStatus_TAPA=='4' }">
			...
			<c:forEach var="TAPUnionPPModel" items="${TAPUnionPPModelList }">
				<tr>
					...
					<c:if test="${sessionScope.checkOutStatus_TAPA=='4' }">
						<c:if test="${TAPUnionPPModel.TAPeriodical.checkOut=='0' }">
							<td><font color="orange" >待所长审核项目</td>
						</c:if>
						<c:if test="${TAPUnionPPModel.TAPeriodical.checkOut=='1' }">
							<td><font color="blue" >待管理员审核</td>
						</c:if>
						<c:if test="${TAPUnionPPModel.TAPeriodical.checkOut=='2' }">
							<td><font color="red">未通过审核</td>
						</c:if>				
						<c:if test="${TAPUnionPPModel.TAPeriodical.checkOut=='3' }">
							<td><font color="green">管理员审核通过</td>
						</c:if>				
					</c:if>
				</tr>
