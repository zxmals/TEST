...
<html lang="zh-CN">
  ...
  <base target="_self"> 
    ...
    <title>ScientficResearchProject --Personal-Set</title>
...
<body class="gray-bg"  onload="DoCheck()">
	<div class="datepick">
		<span>选择日期范围</span>
		<div>
			<form action="GTscientificresearchpro-personset!getPersonJoin?pagenum=1" method="post" name="pickdate" id="pickdates">
	                ...
	                <div class="ibox float-e-margins">
	                    ...
	                    <br>
	                       ...
	                       <table  id="tb" class="table table-striped table-bordered table-hover dataTables-example">
	                            ...
	                            <tbody>
									<c:forEach var="ebj" items="${teacherandsrp }">
										<tr>
											<td>${ebj.scientificResearchProject.srprojectId }</td>
											<td>${ebj.scientificResearchProject.srpname }</td>
											<td>${ebj.scientificResearchProject.projectNumber }</td>
											<td>${ebj.scientificResearchProject.admitedProjectYear }</td>
											<td>${ebj.scientificResearchProject.chargePersonId }</td>
											<td>${teachermp[ebj.scientificResearchProject.chargePersonId] }</td>
											...
											<c:if test="${ebj.scientificResearchProject.chargePersonId!=teacher.teacherId }">
											...
											<c:if test="${ebj.scientificResearchProject.chargePersonId==teacher.teacherId }">
												<td><c:if test="${ebj.checkOut==3 }">&nbsp;&nbsp;&nbsp;√</c:if></td>
											</c:if>
										</tr>
									</c:forEach>
	                            </tbody>                           
	                        ...
	                        <div style="text-align: center;">
	                        	(共查询到${sumrow }记录)&nbsp;&nbsp;&nbsp;&nbsp;
	                        	第${pagenum }/${sumpage }页&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTscientificresearchpro-personset!getPersonJoin?pagenum=1">首页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTscientificresearchpro-personset!getPersonJoin?pagenum=${prepage }">上一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTscientificresearchpro-personset!getPersonJoin?pagenum=${nextpage }">下一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTscientificresearchpro-personset!getPersonJoin?pagenum=${sumpage }">尾页</a>
	                        </div>
	                   </div>
	                </div>
	            </div>
    ...
    $('#changelength').change(function() {
		...
		window.location.replace("GTscientificresearchpro-personset!getPersonJoin?pagenum=1&limit="+$(this).val().trim()+"&foredate="+$('#date1').val().trim()+"&afterdate="+$('#date2').val().trim());
	});
	//退出
	$('.quitpaper').click(function() {
		...
		swal({   title: "确定退出?",   
				 ...
				 function(isConfirm){   
					 if (isConfirm) {     
						 $.post("GTscientificresearchpro-personset!quitProject",
									{"scienrhprojec.srprojectId":row[0].cells[0].innerHTML.trim()},
