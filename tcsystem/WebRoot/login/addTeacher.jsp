...
<body class="gray-bg" onload="DoCheck()">
								...
								<table id="tb"
									...
									<tbody>
										<c:forEach var="ebj" items="${researchLabList }">
											<tr>
												<td>${ebj.researchLabId }</td>
												<td>${ebj.researchLabName}</td>
												<td>${ebj.researchLabAdmin }</td>
					...
					<div class="modal-body">
						<div class="row">
							...
							<div class="form-group">
								<label>教学研究资金规模:</label> <input id="upinfscope" type="text"
    ...
    $('#subupdate').click(function() {
		if($('#upinfscope').val().trim()!=""){//$('#upinfscope').val().trim()!=""
			$.post("ATteachRearchFundLevelset!updateEntity",
					{"entity.fundLevelId":$('#upinfID').val().trim(),
					 "entity.fundLevel":$('#upinfscope').val().trim(),
					 "entity.score":$('#upinfratio').val().trim()},
					function(data,status){
						if(status=="success"){
							if(data.trim()=="succ"){
								swal("Good job!","更新成功","success");
								window.location.replace("ATteachRearchFundLevelset!entityList");
							}else{
	...
	$('.delinf').click(function() {
		 ...
		 swal({
			...
			function(isConfirm){
			  if (isConfirm) {
				  $.post("ATteachRearchFundLevelset!deleteEntity",
							{"entity.fundLevelId":row[0].cells[0].innerHTML,
							 "entity.fundLevel":row[0].cells[1].innerHTML,
