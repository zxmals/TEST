			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<form action="ATTeachingAbilityImprovePerformanceSet!getAllRecord">
							<div>
								<a>每页 <select id="changelength" style="width:80px;height:25px;border-radius:3px;" name="pageSize_ATTAI">
								...
								<select id="termSelection" name="termId_ATTAI" style="width:120px;height:25px;border-radius:3px;">
								...
								<input type="text" id="searchCondition_ATTAI" name="searchCondition_ATTAI"  placeholder="请输入活动的名称"
						...
						<div class="example">
							...
							<div style="text-align: center;">
								(共查询到${sessionScope.recordNumber_ATTAI }记录)&nbsp;&nbsp;&nbsp;&nbsp; 第${pageIndex }/${sessionScope.pageCount_ATTAI }页&nbsp;&nbsp;&nbsp;
								<a class="comphref"
									href="ATTeachingAbilityImprovePerformanceSet!getAllRecord">首页</a>&nbsp;&nbsp;&nbsp;
								<c:if test="${pageIndex>1 }">
									<a class="comphref"
										href="ATTeachingAbilityImprovePerformanceSet!getAllRecord?isDivided=true&pageIndex=${pageIndex-1 }">上一页</a>&nbsp;&nbsp;&nbsp;
								</c:if>
								<c:forEach var="index" begin="${pageIndex }" end="${pageIndex+4 }" step="1">
									<c:if test="${index<pageCount_ATTAI }">
										<a class="comphref"
										href="ATTeachingAbilityImprovePerformanceSet!getAllRecord?isDivided=true&pageIndex=${index }">${index }</a>&nbsp;&nbsp;&nbsp;
									</c:if>
								</c:forEach>
								<c:if test="${pageIndex<pageCount_ATTAI }">
									<a class="comphref"
										href="ATTeachingAbilityImprovePerformanceSet!getAllRecord?isDivided=true&pageIndex=${pageIndex+1 }">下一页</a>&nbsp;&nbsp;&nbsp;
								</c:if>
								<a class="comphref"
									href="ATTeachingAbilityImprovePerformanceSet!getAllRecord?isDivided=true&pageIndex=${pageCount_ATTAI }">尾页</a>
							</div>
						</div>
					</div>
				</div>
							...
							<form role="form" name="" id="updateInfoForm"
								...
								<div class="form-group">
									<label>教师工号</label>&nbsp;<label></label> <input id="up_teacherId"
										type="text" class="form-control" name="teacher.teacherId" class="doCheck_update"
										value="" readonly="readonly">
								</div>
								<div class="form-group">
...
<!-- 		<script src="js/PublicCheck/PUB_SET.js"></script> -->
	...
	<script type="text/javascript">
	$().ready(function(){
		$("#changelength option[value='${sessionScope.pageSize_ATTAI}']").attr("selected",true);
		$("#termSelection option[value='${sessionScope.termId_ATTAI}']").attr("selected",true);
		$("#add_termSelection option[value='${sessionScope.termId_ATTAI}']").attr("selected",true);
		$("#up_termSelection option[value='${sessionScope.termId_ATTAI}']").attr("selected",true);
		$("#searchCondition_ATTAI").val('${sessionScope.searchCondition_ATTAI}');
	});
    $('#updateInfo').click(function() {
    	submitUpdatedInfo("ATTeachingAbilityImprovePerformanceSet", "updateRecord", "getAllRecord");
	});
    ...
    $('.deleteInfo').click(function() {
		...
		}, "ATTeachingAbilityImprovePerformanceSet", "deleteRecord",
