...
<body class="gray-bg" onload="DoCheck()">
	<div class="datepick">
		<span>选择日期范围</span>
		<div>
			<form action="ATva_actManage!getWorkall?pagenum=1" method="post"
				name="pickdate" id="pickdates">
				从:<input type="text" id="date1" class="Wdate"
					...
					type="submit" id="datep" value="查寻" title="点击查询">
			</form>
		</div>
	</div>
	<div class="chooseact">
						...
						<div class="ibox-content" style="height:540px;">
							...
							<div class="example">
										...
										<tbody>
											<c:forEach var="VaAddJoinedAct" items="${List }">
												<tr>
													...
													<td><c:if test="${VaAddJoinedAct.aspareTire == '0'}">待审核</c:if>
														<c:if test="${VaAddJoinedAct.aspareTire == '1'}">审核通过</c:if>
														<c:if test="${VaAddJoinedAct.aspareTire == '2'}">不通过</c:if>
													</td>
													<td><c:if test="${VaAddJoinedAct.aspareTire == '0' }">
															<a class="btn btn-primary btn-sm delwords delinf"
								...
								<div style="text-align: center;">
									...
									<a class="comphref"
										href="ATva_actManage!getWorkall?pagenum=${prepage }">上一页</a>&nbsp;&nbsp;&nbsp;
									<a class="comphref"
										href="ATva_actManage!getWorkall?pagenum=${nextpage }">下一页</a>&nbsp;&nbsp;&nbsp;
									<a class="comphref"
										href="ATva_actManage!getWorkall?pagenum=${sumpage }">尾页</a>
								</div>
							</div>
						</div>
					</div>
