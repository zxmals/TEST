...
<%
 ...
 List<String> b =tpdao.getAllTalentProjectName();
	 ...
	 List<ViewRXRCGC> f = new ArrayList<ViewRXRCGC>();
      ...
      if(teacherid!=null){
      id = teacherid;
      f = dao.getAllRXRCGC(id);
      System.out.println("teacherid:"+teacherid);
      }
      else f = dao.getAllRXRCGC(teacherID);
...
<center>
      ...
      <table class="table table-striped"  style="border-collapse:collapse">
          <input type="submit" value="添加绩效"
                 onclick="document.f0.action='DoSTProject!admintmp?tmp1=0&tid=<%=id %>&teacherID=<%=teacherID%>';document.f0.submit();"></input>
      </form>
      <br>
     ...
     <table class="table table-striped"  style="border-collapse:collapse">
		...
		<%if(f!=null)for(int i=0;i<f.size();i++){ %>
			<tr>
				...
				<td><%=f.get(i).getTalentProjectName()%></td>
				<td><%=f.get(i).getTPSelectedYear() %></td>
				<td><%=f.get(i).getScore() %></td>


                <td><input type="submit" value="修改"
                           onclick="document.f1.action='DoSTProject!admintmp?tmp=<%=i%>&tid=<%=id %>&teacherID=<%=teacherID%>';document.f1.submit();window.location.reload(); "></input>
                    <input type="submit" value="删除"
                           onclick="document.f1.action='DoSTProject!admindelSTProject?TeacherASTPid=<%=f.get(i).getTeacherASTPid()%>&TeacherID=<%=f.get(i).getTeacherID()%>';document.f1.submit();"></input>
                </td>
			</tr>
		<%} %>
	</table>
          ...
          <table class="table table-striped"  style="border-collapse:collapse">
			...
			<tr><td>ID</td><td><input type="text" name="TeacherASTPid" value="<%=f.get(g).getTeacherASTPid() %>" readonly="true"/></td></tr>
			<tr><td>人才工程名称</td><td><select name="TalentProjectName"  >
			<option value="<%=f.get(g).getTalentProjectName()%>"  selected><%=f.get(g).getTalentProjectName() %></option>
			     <%for(int i=0;i<b.size();i++){ %>
					<option value="<%=b.get(i)%>"  ><%=b.get(i) %></option>
				<%} %>			
				 </select>
			</td></tr>
			<tr><td>入选年份</td><td><input type="text" name="TPSelectedYear"value="<%=f.get(g).getTPSelectedYear() %>" /></td></tr>
			<tr><td>得分</td><td><input type="text"  name="FinalScore" value="<%=f.get(g).getScore() %>" /></td></tr>
              ...
              <tr>
                  ...
                  <td><input type="submit" value="提交"
                             onclick="document.f.action='DoSTProject!adminupdateSTProject?teacherID=<%=id %>';document.f.submit();"></input>
                  </td>
              </tr>
	</table>
	</form>
	...
	<center>
          ...
          <table class="table table-striped"  style="border-collapse:collapse">
			...
			<tr><td>入选年份</td><td><input type="text" name="TPSelectedYear"/></td></tr>
              <tr>
                  <td>操作</td>
                  <td><input type="submit" value="提交"
                             onclick="document.f2.action='DoSTProject!adminAddSTProject?teacherID=<%=id%>';document.f2.submit();"></input>
                  </td>
              </tr>
	</table>
	</form>
