				...
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<button class="btn  btn-primary openaddm" type="submit"
							data-backdrop="true" data-toggle="modal" data-target="#add" id="submitNewRecord" onclick="resetSelection()">
							<strong>新增学生竞赛指导绩效</strong>
						</button>
						<br>
						<br>
						<form action="GTStudentCompetitionGuidancePerformanceSet!getAllRecord">
							<div>
								<a>每页 <select id="changelength" style="width:80px;height:25px;border-radius:3px;" name="pageSize_GTSCG">
								...
								<select id="termSelection" name="termId_GTSCG" style="width:120px;height:25px;border-radius:3px;">
						...
						<div class="example">
							...
							<div style="text-align: center;">
								(共查询到${sessionScope.recordNumber_GTSCG }记录)&nbsp;&nbsp;&nbsp;&nbsp; 第${pageIndex }/${sessionScope.pageCount_GTSCG }页&nbsp;&nbsp;&nbsp;
								<a class="comphref"
									href="GTStudentCompetitionGuidancePerformanceSet!getAllRecord">首页</a>&nbsp;&nbsp;&nbsp;
								<c:if test="${pageIndex>1 }">
									<a class="comphref"
										href="GTStudentCompetitionGuidancePerformanceSet!getAllRecord?isDivided=true&pageIndex=${pageIndex-1 }">上一页</a>&nbsp;&nbsp;&nbsp;
								</c:if>
								<c:forEach var="index" begin="${pageIndex }" end="${pageIndex+4 }" step="1">
									<c:if test="${index<pageCount_GTSCG }">
										<a class="comphref"
										href="GTStudentCompetitionGuidancePerformanceSet!getAllRecord?isDivided=true&pageIndex=${index }">${index }</a>&nbsp;&nbsp;&nbsp;
									</c:if>
								</c:forEach>
								<c:if test="${pageIndex<pageCount_GTSCG }">
									<a class="comphref"
										href="GTStudentCompetitionGuidancePerformanceSet!getAllRecord?isDivided=true&pageIndex=${pageIndex+1 }">下一页</a>&nbsp;&nbsp;&nbsp;
								</c:if>
								<a class="comphref"
									href="GTStudentCompetitionGuidancePerformanceSet!getAllRecord?isDivided=true&pageIndex=${pageCount_GTSCG }">尾页</a>
							</div>
						</div>
					</div>
				</div>
		...
		<div id="add" class="modal fade" aria-hidden="true" tabindex="-1"
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						<div class="row">
							<h3 class="m-t-none m-b">添加学生竞赛指导绩效</h3>
							<form role="form" name="adds" id="addInfoForm"
								action=""
								method="post">
								<div class="form-group">
									<label>学期:</label>&nbsp;<label></label><br>
									<select id="add_termSelection" name="studentCompetGuidPerf.termId" class="form-control">
										<c:forEach var="tfterm" items="${tftermList }">
											<option value="${tfterm.termId }">${tfterm.term }</option>
										</c:forEach>
									</select>
								...
								<div class="form-group">
									<label>竞赛名称:</label>&nbsp;<label></label> <input
										id=add_competitionName " type="text"
								...
								<div class="form-group">
									<label>竞赛级别:</label>&nbsp;<label></label> 
									<select
										id="add_competitionTypeId" class="form-control"  onchange="checkCompetitionTypeSelection('add')"
										name="studentCompetGuidanceCompetitionType.competitionTypeId">
										<c:forEach var="studentCompetitionGuidanceCompetitionType"
											items="${studentCompetitionGuidanceCompetitionTypeList }">
											<option value="${studentCompetitionGuidanceCompetitionType.competitionTypeId }">${studentCompetitionGuidanceCompetitionType.competitionType }</option>
								...
								<div class="form-group">
									<label>获奖级别:</label>&nbsp;<label></label>
									<select
										id="add_rewardLevelId" class="form-control"  onchange="checkRewardLevelSelection('add')"
										name="studentCompetGuidRewardLevel.rewardLevelId">
										<c:forEach var="studentCompetitionGuidanceRewardLevel"
											items="${studentCompetitionGuidanceRewardLevelList }">
											<option value="${studentCompetitionGuidanceRewardLevel.rewardLevelId }">${studentCompetitionGuidanceRewardLevel.rewardLevel }</option>
...
<!-- 		<script src="js/PublicCheck/PUB_SET.js"></script> -->
	...
	$().ready(function(){
		$("#changelength option[value='${sessionScope.pageSize_GTSCG}']").attr("selected",true);
		$("#termSelection option[value='${sessionScope.termId_GTSCG}']").attr("selected",true);
		$("#add_termSelection option[value='${sessionScope.termId_GTSCG}']").attr("selected",true);
		$("#up_termSelection option[value='${sessionScope.termId_GTSCG}']").attr("selected",true);
	});
	...
	$("#submitAddInfo").click(function(){
		//执行提交表单
		submitAddedInfo("GTStudentCompetitionGuidancePerformanceSet", "insertRecord", "getAllRecord");
	});
    $('#updateInfo').click(function() {
    	submitUpdatedInfo("GTStudentCompetitionGuidancePerformanceSet", "updateRecord", "getAllRecord");
	});
    ...
    $('.deleteInfo').click(function() {
		...
		}, "GTStudentCompetitionGuidancePerformanceSet", "deleteRecord",
