...
<html lang="zh-CN">
  ...
  <base target="_self"> 
    ...
    <title>TFfinecourse --project-Set</title>
...
<body class="gray-bg"  onload="DoCheck()">
	                ...
	                <div class="ibox float-e-margins">
	                    ...
	                    <div class="ibox-content" style="height:540px;">
	                    	 <button class="btn  btn-primary openaddm" type="submit" data-backdrop="true" data-toggle="modal" data-target="#utdialog">
	                        	 <strong>新增精品课程</strong>
	                         </button><br><br>
	                    <form id="adjusts" action="GTteachingfinecoursePerformanceSet-project!gainAllProject" method="get">
							<div>
			                    	<a>每页   
	                    ...
	                    <br>
	                       ...
	                       <table  id="tb" class="table table-striped table-bordered table-hover dataTables-example">
	                            ...
	                            <tbody>
									<c:forEach var="ebj" items="${finecourseli }">
										<tr>
											<td>${ebj.courseId }</td>
											<td>${ebj.courseName }</td>
											<td title="${ebj.cooperator }">
												<c:if test="${ebj.cooperator=='1' }">是</c:if>
												<c:if test="${ebj.cooperator=='0' }">否</c:if>
											</td>
											<td title="${ebj.tffineCourseConstructionLevel.courseLevelId }">
												${ebj.tffineCourseConstructionLevel.courseLevel }
											</td>
	                        ...
	                        <div style="text-align: center;">
	                        	(共查询到${sumrow }条记录)&nbsp;&nbsp;&nbsp;&nbsp;
	                        	第${pagenum }/${sumpage }页&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingfinecoursePerformanceSet-project!gainAllProject?pagenum=1">首页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingfinecoursePerformanceSet-project!gainAllProject?pagenum=${prepage }">上一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingfinecoursePerformanceSet-project!gainAllProject?pagenum=${nextpage }">下一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingfinecoursePerformanceSet-project!gainAllProject?pagenum=${sumpage }">尾页</a>
	                        </div>
	                   </div>
	                </div>
	            </div>
	        ...
	        <div class="modal-dialog">
	            <div class="modal-content">
	                <div class="modal-body">
	                    <div class="row">
	                            <h3 class="m-t-none m-b" id="addmodaldialogTitle">新增精品课程</h3>
	                            <h3 class="m-t-none m-b" id="updatemodaldialogTitle">修改精品课程</h3>
	                            <hr >
	                                ...
	                                <div class="form-group">                            
	                                    ...
	                                    <select id="level" class="form-control nullcheck addcheck" >
	                                    	<option></option>
	                                    	<c:forEach items="${courselevel }" var="obj">
	                                    		<option value="${obj.courseLevelId }">${obj.courseLevel }</option>
	                                    	</c:forEach>
	                                    </select>
	                                </div>
	                                <div class="form-group">                            
		...
		if(checkadds()){
			swal({   
	    		...
	    		closeOnCancel: true }, 
	    			function(isConfirm){   
	    				if (isConfirm) {
	    					$.post("GTteachingfinecoursePerformanceSet-project!addProject",
	    							{"finecourseproject.courseName":$('#gainName').val().trim(),
	    							 "finecourselevel.courseLevelId":$('#level').val().trim(),
	    							 "finecourseproject.cooperator":$('#otherAuthorJoin').val().trim(),
	    							...
	    							function(data,status){
	    								 if(status=="success"){
	    									 if(data=="succ"){
	    										 swal("添加成功","","success");
	    	    								 setTimeout(function() {
	    	    									 window.location.replace("GTteachingfinecoursePerformanceSet-project!gainAllProject?pagenum=1");
	    										}, 2000);
	    									 }else{
	    										 swal("成功","","warning");
	    									 }
    	...
    	if(checkadds()&&projectId!=""){	
    			swal({   
    	    		...
    	    		closeOnCancel: true }, 
    	    			function(isConfirm){   
    	    				if (isConfirm) {
    	    					$.post("GTteachingfinecoursePerformanceSet-project!updateProject",
    	    							{"finecourseproject.courseId":projectId,
	    	    						 "finecourseproject.courseName":$('#gainName').val().trim(),
		    							 "finecourselevel.courseLevelId":$('#level').val().trim(),
		    							 "finecourseproject.cooperator":$('#otherAuthorJoin').val().trim(),
		    							 "term.termId":$('#aup_term').val().trim(),
		    							 "finecourseproject.checkOut":$('input[type="radio"][name="proJpeople"]:checked').val().trim()},
    	    	    					function(data,status){
    	    	    						if(status=="success"){
    	    	    							 if(data=="succ"){
    	    	    								 swal("更新成功","","success");
    ...
    $('#subdel').click(function() {
    	swal({   
    		...
    		closeOnCancel: true }, 
    			function(isConfirm){   
    				if (isConfirm) {
    					$.post("GTteachingfinecoursePerformanceSet-project!deleteProject",
    							{"finecourseproject.courseId":$('#projectId').val().trim()},
    							function(data,status){
    								if(status=="success"){
    									if(data=="succ"){
    										swal("删除成功","","success");
    ...
    $('.getMember').click(function() {
		...
		$.post("GTteachingfinecoursePerformanceSet-project!getMember",
				{"finecourseproject.courseId":row[0].cells[0].innerHTML},
				function(data,status){
					var tabs = $('#membtab');
    ...
    $('#subchangescore').click(function() {
		...
		$.post("GTteachingfinecoursePerformanceSet-project!changescore",
				{"mixs":jobj,
				 "sumscore":sumscore,
				 "finecourseproject.courseId":projectId},
		function(data,status){
    ...
    $('.joinProj').click(function(e) {
		...
		swal({
    		...
    		function(isConfirm){
    			if(isConfirm){
    				$.post("GTteachingfinecoursePerformanceSet-project!joinPeoject",
    	    				{"finecourseproject.courseId":row[0].cells[0].innerHTML.trim()},
    	    				function(data,status){
    	        				 if(status=="success"){
    	        					 if(data=="succ"){
    	        						 alert("加入成功");
