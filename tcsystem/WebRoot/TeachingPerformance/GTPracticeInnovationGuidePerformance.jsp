						...
						<div class="example">
							<form method="post" name="f">
								<table id="tb"
									...
									<tbody>
										<c:forEach var="pracInnoGuidPerf"
											items="${pracInnoGuidPerfList }">
											<tr>
												<!-- 项目ID -->
												<td>${pracInnoGuidPerf.projectId }</td>
												<!-- 工号 -->
												<td>${pracInnoGuidPerf.teacher.teacherId }</td>
												<!-- 姓名 -->
												<td>${pracInnoGuidPerf.teacher.teacherName }</td>
												<!-- 项目名称 -->
												<td>${pracInnoGuidPerf.projectName }</td>
												<!-- 论文评估级别 -->
												<td>${pracInnoGuidPerf.tfpracticeInnovationGuideGraduationThesisGuideEvalution.thesisEvaluationLevel }</td>
												<!-- 项目指导级别 -->
												<td>${pracInnoGuidPerf.tfpracticeInnovationGuideLevel.innovatIonguideLevel }</td>
												<!-- 得分 -->
												<td>${pracInnoGuidPerf.finalScore }</td>
												<!-- upid -->
												<td style="display:none;">${pracInnoGuidPerf.upid }</td>
												<!-- 状态 -->
												<c:if test="${pracInnoGuidPerf.checkOut ==0 }">
													<td style="color:blue;">待审核</td>
												</c:if>
												<c:if test="${pracInnoGuidPerf.checkOut ==1 }">
													<td style="color: green;">审核通过</td>
												</c:if>
												<c:if test="${pracInnoGuidPerf.checkOut ==2 }">
...
// 		$('#up_quantityUnit').val($(this).parent().parent()[0].cells[5].innerHTML);
		...
		var temp2=$(this).parent().parent()[0].cells[5].innerHTML;
