...
<html lang="zh-CN">
  ...
  <base target="_self"> 
    ...
    <title>TFteachingAchievement --project-Set</title>
...
<body class="gray-bg"  onload="DoCheck()">
	                ...
	                <div class="ibox float-e-margins">
	                    ...
	                    <div class="ibox-content" style="height:540px;">
	                    	 <button class="btn  btn-primary openaddm" type="submit" data-backdrop="true" data-toggle="modal" data-target="#utdialog">
	                        	 <strong>新增教学成果</strong>
	                         </button><br><br>
	                    <form id="adjusts" action="GTteachingachievePerformanceSet-project!gainAllProject" method="get">
							<div>
			                    	<a>每页   
	                    ...
	                    <br>
	                       ...
	                       <table  id="tb" class="table table-striped table-bordered table-hover dataTables-example">
	                            ...
	                            <tbody>
									<c:forEach var="ebj" items="${achieveproject }">
										<tr>
											<td>${ebj.projectId }</td>
											...
											<td title="${ebj.tfteachingAchievementRewardLevel.rewardId }">${ebj.tfteachingAchievementRewardLevel.rewardName }</td>
	                        ...
	                        <div style="text-align: center;">
	                        	(共查询到${sumrow }条记录)&nbsp;&nbsp;&nbsp;&nbsp;
	                        	第${pagenum }/${sumpage }页&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingachievePerformanceSet-project!gainAllProject?pagenum=1">首页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingachievePerformanceSet-project!gainAllProject?pagenum=${prepage }">上一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingachievePerformanceSet-project!gainAllProject?pagenum=${nextpage }">下一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingachievePerformanceSet-project!gainAllProject?pagenum=${sumpage }">尾页</a>
	                        </div>
	                   </div>
	                </div>
	            </div>
	        ...
	        <div class="modal-dialog">
	            <div class="modal-content">
	                <div class="modal-body">
	                    <div class="row">
	                            <h3 class="m-t-none m-b" id="addmodaldialogTitle">新增教学成果</h3>
	                            <h3 class="m-t-none m-b" id="updatemodaldialogTitle">修改教学成果</h3>
	                            <hr >
	                                ...
	                                <div class="form-group">                            
	                                    ...
	                                    <select id="rewardlevel" class="form-control nullcheck addcheck" >
	                                    	...
	                                    	<c:forEach items="${rewardlevel }" var="obj">
	                                    		<option value="${obj.rewardId }">${obj.rewardName }</option>
	                                    	</c:forEach>
	                                    </select>
	                                </div>
	                                <div class="form-group">                            
		...
		if(checkadds()){
			swal({   
	    		...
	    		closeOnCancel: true }, 
	    			function(isConfirm){   
	    				if (isConfirm) {
	    					$.post("GTteachingachievePerformanceSet-project!addProject",
	    							{"achieveproject.projectName":$('#gainName').val().trim(),
	    							 "achievelevel.rewardId":$('#rewardlevel').val().trim(),
	    							 "achieveproject.cooperator":$('#otherAuthorJoin').val().trim(),
	    							...
	    							function(data,status){
	    								 if(status=="success"){
	    									 if(data=="succ"){
	    										 swal("添加成功","","success");
	    	    								 setTimeout(function() {
	    	    									 window.location.replace("GTteachingachievePerformanceSet-project!gainAllProject?pagenum=1");
	    										}, 2000);
	    									 }else{
	    										 swal("成功","","warning");
	    									 }
    	...
    	if(checkadds()&&projectId!=""){	
    			swal({   
    	    		...
    	    		closeOnCancel: true }, 
    	    			function(isConfirm){   
    	    				if (isConfirm) {
    	    					$.post("GTteachingachievePerformanceSet-project!updateProject",
    	    							{"achieveproject.projectId":projectId,
	    	    						 "achieveproject.projectName":$('#gainName').val().trim(),
		    							 "achievelevel.rewardId":$('#rewardlevel').val().trim(),
		    							 "achieveproject.cooperator":$('#otherAuthorJoin').val().trim(),
		    							 "term.termId":$('#aup_term').val().trim(),
		    							 "achieveproject.checkOut":$('input[type="radio"][name="proJpeople"]:checked').val().trim()},
    	    	    					function(data,status){
    	    	    						if(status=="success"){
    	    	    							 if(data=="succ"){
    	    	    								 swal("更新成功","","success");
    ...
    $('#subdel').click(function() {
    	swal({   
    		...
    		closeOnCancel: true }, 
    			function(isConfirm){   
    				if (isConfirm) {
    					$.post("GTteachingachievePerformanceSet-project!deleteProject",
    							{"achieveproject.projectId":$('#projectId').val().trim()},
    							function(data,status){
    								if(status=="success"){
    									if(data=="succ"){
    										swal("删除成功","","success");
    ...
    $('.getMember').click(function() {
		...
		$.post("GTteachingachievePerformanceSet-project!getMember",
				{"achieveproject.projectId":row[0].cells[0].innerHTML},
				function(data,status){
					var tabs = $('#membtab');
    ...
    $('#subchangescore').click(function() {
		...
		$.post("GTteachingachievePerformanceSet-project!changescore",
				{"mixs":jobj,
				 "sumscore":sumscore,
				 "achieveproject.projectId":projectId},
		function(data,status){
    ...
    $('.joinProj').click(function(e) {
		...
		swal({
    		...
    		function(isConfirm){
    			if(isConfirm){
    				$.post("GTteachingachievePerformanceSet-project!joinPeoject",
    	    				{"achieveproject.projectId":row[0].cells[0].innerHTML.trim()},
    	    				function(data,status){
    	        				 if(status=="success"){
    	        					 if(data=="succ"){
    	        						 alert("加入成功");
