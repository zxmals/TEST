...
<html lang="zh-CN">
  ...
  <base target="_self"> 
    ...
    <title>TFprofession_construction&research_project_applicansion --project-Set</title>
...
<body class="gray-bg"  onload="DoCheck()">
	                ...
	                <div class="ibox float-e-margins">
	                    ...
	                    <div class="ibox-content" style="height:540px;">
	                    	 <button class="btn  btn-primary openaddm" type="submit" data-backdrop="true" data-toggle="modal" data-target="#utdialog">
	                        	 <strong>新增专业建设项目申报</strong>
	                         </button><br><br>
	                    <form id="adjusts" action="GTteachingprofeprojePerformanceSet-project!gainAllProject" method="get">
							<div>
			                    	<a>每页   
	                    ...
	                    <br>
	                       ...
	                       <table  id="tb" class="table table-striped table-bordered table-hover dataTables-example">
	                            ...
	                            <tbody>
									<c:forEach var="ebj" items="${profeprojeporjeli }">
										<tr>
											<td>${ebj.projectId }</td>
											<td>${ebj.projectName }</td>
											<td title="${ebj.tfprofessionalProjectDeclareLevel.projectLevelId }">
												${ebj.tfprofessionalProjectDeclareLevel.projectLevel }
											</td>
	                        ...
	                        <div style="text-align: center;">
	                        	(共查询到${sumrow }条记录)&nbsp;&nbsp;&nbsp;&nbsp;
	                        	第${pagenum }/${sumpage }页&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingprofeprojePerformanceSet-project!gainAllProject?pagenum=1">首页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingprofeprojePerformanceSet-project!gainAllProject?pagenum=${prepage }">上一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingprofeprojePerformanceSet-project!gainAllProject?pagenum=${nextpage }">下一页</a>&nbsp;&nbsp;&nbsp;
	                        	<a class="comphref" href="GTteachingprofeprojePerformanceSet-project!gainAllProject?pagenum=${sumpage }">尾页</a>
	                        </div>
	                   </div>
	                </div>
	            </div>
	                                ...
	                                <div class="form-group">                            
	                                    ...
	                                    <select id="level" class="form-control nullcheck addcheck" >
	                                    	...
	                                    	<c:forEach items="${projectlevel }" var="obj">
	                                    		<option value="${obj.projectLevelId }">${obj.projectLevel }</option>
	                                    	</c:forEach>
	                                    </select>
	                                </div>
	                                <div class="form-group">                            
		...
		if(checkadds()){
			swal({   
	    		...
	    		closeOnCancel: true }, 
	    			function(isConfirm){   
	    				if (isConfirm) {
	    					$.post("GTteachingprofeprojePerformanceSet-project!addProject",
	    							{"profeprojeproject.projectName":$('#gainName').val().trim(),
	    							 "profeprojelevel.projectLevelId":$('#level').val().trim(),
	    							...
	    							function(data,status){
	    								 if(status=="success"){
	    									 if(data=="succ"){
	    										 swal("添加成功","","success");
	    	    								 setTimeout(function() {
	    	    									 window.location.replace("GTteachingprofeprojePerformanceSet-project!gainAllProject?pagenum=1");
	    										}, 2000);
	    									 }else{
	    										 swal("成功","","warning");
	    									 }
    	...
    	if(checkadds()&&projectId!=""){	
    			swal({   
    	    		...
    	    		closeOnCancel: true }, 
    	    			function(isConfirm){   
    	    				if (isConfirm) {
    	    					$.post("GTteachingprofeprojePerformanceSet-project!updateProject",
    	    							{"profeprojeproject.projectId":projectId,
	    	    						 "profeprojeproject.projectName":$('#gainName').val().trim(),
		    							 "profeprojelevel.projectLevelId":$('#level').val().trim(),
		    							 "term.termId":$('#aup_term').val().trim(),
		    							 "profeprojeproject.checkOut":$('input[type="radio"][name="proJpeople"]:checked').val().trim()},
    	    	    					function(data,status){
    	    	    						if(status=="success"){
    	    	    							 if(data=="succ"){
    	    	    								 swal("更新成功","","success");
    ...
    $('#subdel').click(function() {
    	swal({   
    		...
    		closeOnCancel: true }, 
    			function(isConfirm){   
    				if (isConfirm) {
    					$.post("GTteachingprofeprojePerformanceSet-project!deleteProject",
    							{"profeprojeproject.projectId":$('#projectId').val().trim()},
    							function(data,status){
    								if(status=="success"){
    									if(data=="succ"){
    										swal("删除成功","","success");
    ...
    $('.getMember').click(function() {
		...
		$.post("GTteachingprofeprojePerformanceSet-project!getMember",
				{"profeprojeproject.projectId":row[0].cells[0].innerHTML},
				function(data,status){
					var tabs = $('#membtab');
    ...
    $('#subchangescore').click(function() {
		...
		$.post("GTteachingprofeprojePerformanceSet-project!changescore",
				{"mixs":jobj,
				 "sumscore":sumscore,
				 "profeprojeproject.projectId":projectId},
		function(data,status){
    ...
    $('.joinProj').click(function(e) {
		...
		swal({
    		...
    		function(isConfirm){
    			if(isConfirm){
    				$.post("GTteachingprofeprojePerformanceSet-project!joinPeoject",
    	    				{"profeprojeproject.projectId":row[0].cells[0].innerHTML.trim()},
    	    				function(data,status){
    	        				 if(status=="success"){
    	        					 if(data=="succ"){
    	        						 alert("加入成功");
